...
/*
 * mnv9cmds.c: Dealing with commands of a compiled function
 */
...
check_mnv9_unlet(char_u *name)
{
    ...
    if (name[1] != ':' || mnv_strchr((char_u *)"gwtb", *name) == NULL)
    {
	// "unlet s:var" is allowed in legacy script.
	if (*name == 's' && !in_mnv9script())
	    return OK;
    ...
    void    *coookie)
{
    ...
    *name_end = NUL;
    ...
    else if (mnv_strchr(p, '.') != NULL || mnv_strchr(p, '[') != NULL)
    {
	lhs_T	    lhs;
    ...
    else if (check_mnv9_unlet(p) == FAIL)
    {
	ret = FAIL;
    }
    else
    ...
    void    *coookie)
{
    ...
    *name_end = NUL;
    ...
    {
	if (deep < 0)
	    mnv_snprintf((char *)buf, len, "%s! %s", cmd, p);
	else
	    mnv_snprintf((char *)buf, len, "%s %d %s", cmd, deep, p);
#ifdef LOG_LOCKVAR
...
compile_for(char_u *arg_start, cctx_T *cctx)
{
	...
	for (idx = 0; idx < var_count; ++idx)
	{
	    ...
	    if (get_var_dest(name, &dest, CMD_for, &opt_flags,
					      &mnvvaridx, &type, cctx) == FAIL)
		goto failed;
	    if (dest != dest_local)
	    {
		if (generate_store_var(cctx, dest, opt_flags, mnvvaridx,
						     type, name, NULL) == FAIL)
		    goto failed;
	    }
...
compile_catch(char_u *arg, cctx_T *cctx)
{
	...
	if (tofree == NULL)
	    ...
	    len = (int)(end - tofree);
	pat = mnv_strnsave(tofree == NULL ? p + 1 : tofree, len);
	mnv_free(tofree);
	p += len + 2 + dropped;
	if (pat == NULL)
	    return NULL;
...
compile_variable_range(exarg_T *eap, cctx_T *cctx)
{
    ...
    if (p == range_end)
	return FAIL;
    return generate_RANGE(cctx, mnv_strnsave(eap->cmd, range_end - eap->cmd));
}
...
compile_exec(char_u *line_arg, exarg_T *eap, cctx_T *cctx)
{
	...
	for (;;)
	{
	    if (p > start)
	    {
		char_u *val = mnv_strnsave(start, p - start);
...
compile_substitute(char_u *arg, exarg_T *eap, cctx_T *cctx)
{
	...
	if (cmd[0] == delimiter && cmd[1] == '\\' && cmd[2] == '=')
	{
	    ...
	    isn->isn_arg.subs.subs_cmd = mnv_strsave(arg);
...
compile_redir(char_u *line, exarg_T *eap, cctx_T *cctx)
{
	...
	if (STRNCMP(arg, "END", 3) == 0)
	{
		...
		MNV_CLEAR(lhs->lhs_name);
		MNV_CLEAR(lhs->lhs_whole);
	    }
	    return arg + 3;
	}
	...
	else
	{
	    ...
	    if (lhs->lhs_has_index)
	    {
		lhs->lhs_whole = mnv_strnsave(arg, lhs->lhs_varlen_total);
...
compile_cexpr(char_u *line, exarg_T *eap, cctx_T *cctx)
{
    ...
    isn->isn_arg.cexpr.cexpr_ref->cer_cmdline = mnv_strsave(skipwhite(line));
...
{
    if (arg == cmd + 1 && mnv_strchr((char_u *)":-.", *arg) != NULL)
    {
	semsg(_(e_separator_not_supported_str), arg);
	return FAIL;
    }
    if (MNV_ISWHITE(cmd[1]))
    {
