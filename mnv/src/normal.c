...
// Include the lookuptable generated by create_nvcmdidx.mnv.
	...
	int	    *need_flushbuf UNUSED)
{
	...
	*need_flushbuf |= add_to_showcmd(*cp);
	...
	if (enc_utf8 && lang)
	{
	    ...
	    ++no_mapping;
	    // MNV may be in a different mode when the user types the next key,
...
# endif
    ...
    if (VIsual_active
	    && VIsual_select
	    && (mnv_isprintc(c) || c == NL || c == CAR || c == K_KENTER))
    {
	int len;
    ...
    int		find_type)
{
	...
	if (has_mbyte)
	{
	    ...
	    while (ptr[col] != NUL
		    && (i == 0 ? !mnv_iswordc(ptr[col]) : MNV_ISWHITE(ptr[col]))
	...
	else
	{
	    while (col > 0
		    && ((i == 0
			    ? mnv_iswordc(ptr[col - 1])
			    : (!MNV_ISWHITE(ptr[col - 1])
				&& (!(find_type & FIND_IDENT)
				    || !mnv_iswordc(ptr[col - 1]))))
			|| ((find_type & FIND_EVAL)
			    && col > 1
	    ...
	    // If we don't want just any old text, or we've found an
	    // identifier, stop searching.
	    if (!(find_type & FIND_STRING) || mnv_iswordc(ptr[col]))
		break;
	}
    }

    if (ptr[col] == NUL || (i == 0
		&& (has_mbyte ? this_class != 2 : !mnv_iswordc(ptr[col]))))
    {
	...
	*textcol = col;
	...
	while ((i == 0 ? mnv_iswordc(ptr[col])
		       : (ptr[col] != NUL && !MNV_ISWHITE(ptr[col])))
		    || ((find_type & FIND_EVAL)
			&& col <= (int)startcol
			&& find_is_eval_item(ptr + col, &col, &bn, FORWARD))
		)
...
#endif
	    mnv_snprintf((char *)showcmd_buf, SHOWCMD_BUFLEN, "%ldx%ld", lines,
					      (long)(rightcol - leftcol + 1));
	}
	else if (VIsual_mode == 'V' || VIsual.lnum != curwin->w_cursor.lnum)
	    mnv_snprintf((char *)showcmd_buf, SHOWCMD_BUFLEN, "%ld", lines);
	else
	{
	    char_u  *s, *e;
	    int	    l;
	    ...
	    if (bytes == chars)
		mnv_snprintf((char *)showcmd_buf, SHOWCMD_BUFLEN, "%d", chars);
	    else
		mnv_snprintf((char *)showcmd_buf, SHOWCMD_BUFLEN, "%d-%d", chars, bytes);
	}
...
{
    int	    len = mnv_strsize(showcmd_buf);
...
do_check_scrollbind(int check)
{
	...
	else if (mnv_strchr(p_sbo, 'j')) // jump flag set in 'scrollopt'
	{
...
check_scrollbind(linenr_T topline_diff, long leftcol_diff)
{
    ...
    want_ver = (mnv_strchr(p_sbo, 'v') && topline_diff != 0);
#ifdef FEAT_DIFF
    want_ver |= old_curwin->w_p_diff;
#endif
    want_hor = (mnv_strchr(p_sbo, 'h') && (leftcol_diff || topline_diff != 0));
    ...
    int		flags_arg)	// flags passed to searchit()
{
    ...
    // and "~" causes trouble.
    patlen = mnv_snprintf((char *)pat, len + 7,
	    mnv_iswordp(ptr) ? "\\V\\<%.*s\\>" : "\\V%.*s", len, ptr);
...
nv_z_get_count(cmdarg_T *cap, int *nchar_arg)
{
	...
	if (nchar == K_DEL || nchar == K_KDEL)
	    n /= 10;
	else if (MNV_ISDIGIT(nchar))
	{
	    if (mnv_append_digit_long(&n, nchar - '0') == FAIL)
	    {
		clearopbeep(cap->oap);
		break;
	    }
...
nv_zg_zw(cmdarg_T *cap, int nchar)
{
	...
	if (mnv_strchr((char_u *)"gGwW", nchar) == NULL)
	{
	    clearopbeep(cap->oap);
	    return OK;
	}
...
nv_zet(cmdarg_T *cap)
{
    ...
    if (MNV_ISDIGIT(nchar) && !nv_z_get_count(cap, &nchar))
	    return;
    ...
    if (
	    ...
	    && !(VIsual_active && mnv_strchr((char_u *)"dcCoO", cap->nchar))
...
handle_tabmenu(void)
{
	...
	case TABLINE_MENU_CLOSE:
	    ...
	    {
		mnv_snprintf((char *)IObuff, IOSIZE, "confirm tabclose %d",
								 current_tab);
	...
	case TABLINE_MENU_NEW:
	    ...
	    {
		mnv_snprintf((char *)IObuff, IOSIZE, "%dtabnew",
							     current_tab - 1);
		do_cmdline_cmd(IObuff);
	    }
	    break;
	...
	case TABLINE_MENU_OPEN:
	    ...
	    {
		mnv_snprintf((char *)IObuff, IOSIZE, "browse %dtabnew",
							     current_tab - 1);
		do_cmdline_cmd(IObuff);
	    }
	    break;
	...
	size_t		*buflen)
{
	...
	*buflen = STRLEN_LITERAL("he! ");
	...
	if (cap->count0 != 0)
	    *buflen = mnv_snprintf((char *)buf, bufsize, "%s %ld ", kp, cap->count0);
	else
	    *buflen = mnv_snprintf((char *)buf, bufsize, "%s ", kp);
	return n;
    }
	...
	*buflen = mnv_snprintf((char *)buf, bufsize, ".,.+%ld! ", cap->count0 - 1);
    else
	*buflen = mnv_snprintf((char *)buf, bufsize, "! ");

    if (cap->count0 == 0 && isman_s)
	*buflen += mnv_snprintf((char *)buf + *buflen, bufsize - *buflen, "man ");
    else
	*buflen += mnv_snprintf((char *)buf + *buflen, bufsize - *buflen, "%s ", kp);
    if (cap->count0 != 0 && (isman || isman_s))
	*buflen += mnv_snprintf((char *)buf + *buflen, bufsize - *buflen, "%ld ", cap->count0);
...
nv_ident(cmdarg_T *cap)
{
	...
	default:
		...
		else if (cap->count0 == 0)
		{
		    ...
		    buflen = mnv_snprintf((char *)buf, bufsize, ":%ldta ", cap->count0);
	    }
    }
	...
	ptr = mnv_strnsave(ptr, n);
	if (kp_ex)
	    // Escape the argument properly for an Ex command
	    p = mnv_strsave_fnameescape(ptr, VSE_NONE);
	else
	    // Escape the argument properly for a shell command
	    p = mnv_strsave_shellescape(ptr, TRUE, TRUE);
	mnv_free(ptr);
	if (p == NULL)
	{
	    mnv_free(buf);
	...
	plen = STRLEN(p);
	newbuf = mnv_realloc(buf, buflen + plen + 1);
	if (newbuf == NULL)
	{
	    mnv_free(buf);
	    mnv_free(p);
	...
	while (n-- > 0)
	{
	    ...
	    if (mnv_strchr(aux_ptr, *ptr) != NULL)
		*p++ = '\\';
	...
	*p = NUL;
    ...
    {
	if (!g_cmd && (has_mbyte
		    ? mnv_iswordp(mb_prevptr(ml_get_curline(), ptr))
		    : mnv_iswordc(ptr[-1])))
	{
	    STRCPY(buf + buflen, "\\>");
	    buflen += STRLEN_LITERAL("\\>");
...
nv_right(cmdarg_T *cap)
{
		...
		)
	{
	    ...
	    if (       ((cap->cmdchar == ' '
			    && mnv_strchr(p_ww, 's') != NULL)
			|| (cap->cmdchar == 'l'
			    && mnv_strchr(p_ww, 'l') != NULL)
			|| (cap->cmdchar == K_RIGHT
			    && mnv_strchr(p_ww, '>') != NULL))
		    && curwin->w_cursor.lnum < curbuf->b_ml.ml_line_count)
	    {
...
nv_left(cmdarg_T *cap)
{
	...
	if (oneleft() == FAIL)
	{
	    ...
	    if (       (((cap->cmdchar == K_BS
				|| cap->cmdchar == Ctrl_H)
			    && mnv_strchr(p_ww, 'b') != NULL)
			|| (cap->cmdchar == 'h'
			    && mnv_strchr(p_ww, 'h') != NULL)
			|| (cap->cmdchar == K_LEFT
			    && mnv_strchr(p_ww, '<') != NULL))
		    && curwin->w_cursor.lnum > 1)
	    {
...
nv_brackets(cmdarg_T *cap)
{
    ...
    // define	      "]d"  "[d"   "]D"  "[D"	"]^D"  "[^D"
    if (mnv_strchr((char_u *)"iI\011dD\004", cap->nchar) != NULL)
    {
	char_u	*ptr;
	int	len;
    ...
    // "[M" or "]M" search for prev/next end of (Java) method.
    if (  (cap->cmdchar == '['
		&& mnv_strchr((char_u *)"{(*/#mM", cap->nchar) != NULL)
	    || (cap->cmdchar == ']'
		&& mnv_strchr((char_u *)"})*/#mM", cap->nchar) != NULL))
...
n_swapchar(cmdarg_T *cap)
{
    ...
    if (LINEEMPTY(curwin->w_cursor.lnum) && mnv_strchr(p_ww, '~') == NULL)
    {
	clearopbeep(cap->oap);
	return;
    }
...
v_visop(cmdarg_T *cap)
{
    ...
    cap->cmdchar = *(mnv_strchr(trans, cap->cmdchar) + 1);
    nv_operator(cap);
}
...
nv_optrans(cmdarg_T *cap)
{
	...
	if (cap->cmdchar == 'D' && mnv_strchr(p_cpo, CPO_HASH) != NULL)
	{
	...
	else
	{
	    ...
	    stuffReadbuff(ar[(int)(mnv_strchr(str, cap->cmdchar) - str)]);
	}
    }
    cap->opcount = 0;
}
...
nv_g_home_m_cmd(cmdarg_T *cap)
{
	...
	do
	    i = gchar_cursor();
	while (MNV_ISWHITE(i) && oneright() == OK);
...
nv_g_underscore_cmd(cmdarg_T *cap)
{
    ...
    while (curwin->w_cursor.col > 0
	    && MNV_ISWHITE(ptr[curwin->w_cursor.col]))
	--curwin->w_cursor.col;
    curwin->w_set_curswant = TRUE;
    adjust_for_sel(cap);
}
...
n_opencmd(cmdarg_T *cap)
{
	...
	if (mnv_strchr(p_cpo, CPO_HASH) != NULL)
	    cap->count1 = 1;
...
{
    if (optype == OP_NOP)
	set_mnv_var_string(VV_OP, NULL, 0);
    else
    {
	char_u	opchars[3];
	...
	opchars[2] = NUL;
	set_mnv_var_string(VV_OP, opchars, 2);
    }
}
#endif
...
nv_wordcmd(cmdarg_T *cap)
{
	...
	if (n != NUL)			// not an empty line
	{
		...
		// in 'cpoptions'.
		if (cap->count1 == 1 && mnv_strchr(p_cpo, CPO_CW) != NULL)
		{
		    cap->oap->inclusive = TRUE;
		    cap->oap->motion_type = MCHAR;
		    return;
		...
		if (mnv_strchr(p_cpo, CPO_WORD) != NULL)
		{
		    cap->oap->inclusive = TRUE;
		    word_end = TRUE;
		}
...
nv_esc(cmdarg_T *cap)
{
						...
						&& !VIsual_active && no_reason)
	{
	    ...
	    // The user may accidentally do "mnv file | grep word" and then
	    // CTRL-C doesn't show anything.  With a changed buffer give the
	    // message on stderr.  Without any changes might as well exit.
	    if (anyBufIsChanged())
	    {
		char *ms = _("Type  :qa!  and press <Enter> to abandon all changes and exit MNV");
		...
		if (out_redir)
		{
		    ...
		    msg(_("Type  :qa  and press <Enter> to exit MNV"));
	    }
	}
...
nv_edit(cmdarg_T *cap)
{
	...
	switch (cap->cmdchar)
	{
		...
		if (mnv_strchr(p_cpo, CPO_INSEND) == NULL)
		    beginline(BL_WHITE);
		else
		    beginline(BL_WHITE|BL_FIX);
		break;
