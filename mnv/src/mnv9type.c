...
{
    while (gap->ga_len > 0)
	mnv_free(((type_T **)gap->ga_data)[--gap->ga_len]);
    ga_clear(gap);
}
 ...
 */
    type_T *
typval2type_mnvvar(typval_T *tv, garray_T *type_gap)
{
...
type_name_list_or_dict(char *name, type_T *type, char **tofree)
{
    ...
    *tofree = alloc(len);
    if (*tofree == NULL)
    {
	mnv_free(member_free);
	return name;
    }

    mnv_snprintf(*tofree, len, "%s<%s>", name, member_name);
    mnv_free(member_free);
    return *tofree;
}
...
type_name_class_or_obj(char *name, type_T *type, char **tofree)
{
    ...
    *tofree = alloc(len);
    ...
    mnv_snprintf(*tofree, len, "%s<%s>", name, class_name.string);
    return *tofree;
}
...
f_typename(typval_T *argvars, typval_T *rettv)
{
    ...
    if (argvars[0].v_type == VAR_TYPEALIAS)
    {
	mnv_snprintf((char *)IObuff, IOSIZE, "typealias<%s>", name);
	rettv->vval.v_string = mnv_strsave((char_u *)IObuff);
	if (tofree != NULL)
	    mnv_free(tofree);
    }
    else
	...
	if (tofree != NULL)
	    ...
	    rettv->vval.v_string = mnv_strsave((char_u *)name);
    }
    clear_type_list(&type_list);
}
