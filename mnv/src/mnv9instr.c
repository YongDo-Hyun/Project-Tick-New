...
/*
 * mnv9instr.c: Dealing with instructions of a compiled function
 */
...
generate_PUSHFUNC(cctx_T *cctx, char_u *name, type_T *type, int may_prefix)
{
	...
	funcname = NULL;
	    ...
	    || mnv_strchr(name, AUTOLOAD_CHAR) != NULL)	    // autoload
	funcname = mnv_strsave(name);
    else
    {
...
generate_AUTOLOAD(cctx_T *cctx, char_u *name, type_T *type)
{
	...
	return FAIL;
    isn->isn_arg.string = mnv_strsave(name);
	...
	int	    opt_flags)
{
    ...
    if ((isn = generate_instr_drop(cctx, isn_type, 1)) == NULL)
	return FAIL;
    isn->isn_arg.storeopt.so_name = mnv_strsave(name);
...
{
    int	    di_flags;
    int	    vidx = find_mnv_var(name, &di_flags);
    type_T  *type;
    ...
    type = get_mnv_var_type(vidx, cctx->ctx_type_list);
...
generate_UNLET(cctx_T *cctx, isntype_T isn_type, char_u *name, int forceit)
{
    ...
    if ((isn = generate_instr(cctx, isn_type)) == NULL)
	return FAIL;
    isn->isn_arg.unlet.ul_name = mnv_strsave(name);
	...
	type_T	    *type)
{
    ...
    if (isn == NULL)
	return FAIL;
    isn->isn_arg.loadstore.ls_name = mnv_strsave(name);
	...
	int	    *isn_idx)
{
	...
	*isn_idx = cctx->ctx_instr.ga_len - 1;
	...
	extra->fre_func_name = mnv_strnsave(ufunc->uf_name, ufunc->uf_namelen);
    if (ufunc->uf_def_status != UF_NOT_COMPILED && cl == NULL)
    {
...
generate_DEF(cctx_T *cctx, char_u *name, size_t len)
{
    ...
    if (len > 0)
    {
	isn->isn_arg.string = mnv_strnsave(name, len);
	if (isn->isn_arg.string == NULL)
	    return FAIL;
    }
    return OK;
	...
	int	    is_super)
{
	...
	isn->isn_arg.ufunc.cuf_name = mnv_strnsave(ufunc->uf_name, ufunc->uf_namelen);
...
generate_UCALL(cctx_T *cctx, char_u *name, int argcount)
{
	...
	return FAIL;
    isn->isn_arg.ufunc.cuf_name = mnv_strsave(name);
    isn->isn_arg.ufunc.cuf_argcount = argcount;
...
generate_LOCKUNLOCK(cctx_T *cctx, char_u *line, int is_arg)
{
    ...
    isn->isn_arg.lockunlock.lu_string = mnv_strsave(line);
...
generate_EXEC_copy(cctx_T *cctx, isntype_T isntype, char_u *line)
{
    ...
    if ((isn = generate_instr(cctx, isntype)) == NULL)
	return FAIL;
    isn->isn_arg.string = mnv_strsave(line);
    return OK;
}
	...
	lhs_T		*lhs)
{
	...
	case dest_global:
	    ...
	    return generate_STORE(cctx, mnv_strchr(name, AUTOLOAD_CHAR) == NULL
	...
	case dest_reg:
	    return generate_STORE(cctx, ISN_STOREREG,
					 name[1] == '@' ? '"' : name[1], NULL);
	case dest_mnvvar:
	    return generate_STORE(cctx, ISN_STOREV, mnvvaridx, NULL);
	case dest_script:
	case dest_script_v9:
	    {
		...
		if (scriptvar_idx < 0)
		{
		    ...
		    // ISN_STORES) if destination is in a mnv9script or if
		    ...
		    {
			// "import autoload './dir/script.mnv'" - load script
			// first
		...
		return generate_MNV9SCRIPT(cctx, ISN_STORESCRIPT,
...
delete_instr(isn_T *isn)
{
	...
	case ISN_STRINGMEMBER:
	    mnv_free(isn->isn_arg.string);
	    break;

	case ISN_LOCKUNLOCK:
	    class_unref(isn->isn_arg.lockunlock.lu_cl_exec);
	    mnv_free(isn->isn_arg.lockunlock.lu_string);
	    break;
	...
	case ISN_SUBSTITUTE:
		...
		mnv_free(isn->isn_arg.subs.subs_cmd);
	...
	case ISN_STOREEXPORT:
	    mnv_free(isn->isn_arg.loadstore.ls_name);
	    break;

	case ISN_UNLET:
	case ISN_UNLETENV:
	    mnv_free(isn->isn_arg.unlet.ul_name);
	    break;

	case ISN_STOREOPT:
	case ISN_STOREFUNCOPT:
	    mnv_free(isn->isn_arg.storeopt.so_name);
	    break;
	...
	case ISN_UCALL:
	    mnv_free(isn->isn_arg.ufunc.cuf_name);
	    break;
	...
	case ISN_NEWFUNC:
		...
		if (arg != NULL)
		{
		    ...
		    mnv_free(arg->nfa_lambda);
		    mnv_free(arg->nfa_global);
		    mnv_free(arg);
		}
	    }
	    break;
	...
	case ISN_CMDMOD:
	    mnv_regfree(isn->isn_arg.cmdmod.cf_cmdmod
		    ->cmod_filter_regmatch.regprog);
	    mnv_free(isn->isn_arg.cmdmod.cf_cmdmod);
	    break;

	case ISN_LOADSCRIPT:
	case ISN_STORESCRIPT:
	    mnv_free(isn->isn_arg.script.scriptref);
	    break;
	...
	case ISN_TRY:
	    mnv_free(isn->isn_arg.tryref.try_ref);
	    break;

	case ISN_CEXPR_CORE:
	    mnv_free(isn->isn_arg.cexpr.cexpr_ref->cer_cmdline);
	    mnv_free(isn->isn_arg.cexpr.cexpr_ref);
	    break;
