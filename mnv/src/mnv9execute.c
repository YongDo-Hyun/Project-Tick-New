...
/*
 * mnv9execute.c: execute MNV9 script instructions
 */
...
func_return(ectx_T *ectx)
{
	...
	if (ectx->ec_outer_ref->or_outer_allocated)
	    mnv_free(ectx->ec_outer_ref->or_outer);
	partial_unref(ectx->ec_outer_ref->or_partial);
	mnv_free(ectx->ec_outer_ref);
    }
 ...
 */
    static int
mnv9_aborting(int prev_uncaught_emsg)
{
	...
	dict_T	    *selfdict)
{
	...
	*start_bracket = NUL;
	...
	if (mnv9_aborting(prev_uncaught_emsg))
	{
	    if (start_bracket != NULL)
		*start_bracket = cc;
	...
	switch (tv->v_type)
	{
	    ...
	    case VAR_LIST:
				if (tostring_flags & TOSTRING_TOLERANT)
				{
					...
					return FAIL;
				    s = str;
				    while ((e = mnv_strchr(s, '\n')) != NULL)
				    {
					*e = NUL;
					p = mnv_strsave_fnameescape(s,
						VSE_NONE);
					if (p != NULL)
					{
					    ga_concat(&ga, p);
...
char_from_string(char_u *str, varnumber_T index)
{
    ...
    if (nbyte >= slen)
	return NULL;
    return mnv_strnsave(str + nbyte, mb_ptr2len(str + nbyte));
}
...
string_slice(char_u *str, varnumber_T first, varnumber_T last, int exclusive)
{
	...
	return NULL;
    return mnv_strnsave(str + start_byte, end_byte - start_byte);
}
	...
	getline_opt_T options UNUSED)
{
    ...
    p = mnv_strchr(sp->nextline, '\n');
    if (p == NULL)
    {
	line = mnv_strsave(sp->nextline);
	sp->nextline += STRLEN(sp->nextline);
    }
    else
    {
	line = mnv_strnsave(sp->nextline, p - sp->nextline);
	sp->nextline = p + 1;
    }
    return line;
}
...
load_namespace_var(ectx_T *ectx, isntype_T isn_type, isn_T *iptr)
{
	...
	SOURCING_LNUM = iptr->isn_lnum;
	if (mnv_strchr(iptr->isn_arg.string, AUTOLOAD_CHAR) != NULL)
...
exec_instructions(ectx_T *ectx)
{
	...
	switch (iptr->isn_type)
	{
		...
		if (GA_GROW_FAILS(&ectx->ec_stack, 1))
		    goto theend;
		copy_tv(get_mnv_var_tv(iptr->isn_arg.number), STACK_TV_BOT(0));
		++ectx->ec_stack.ga_len;
		break;

	    // load s: variable in MNV9 script
	    case ISN_LOADSCRIPT:
		{
		...
		*tv = *STACK_TV_BOT(0);
	    ...
	    // store script-local variable in MNV9 script
		...
		--ectx->ec_stack.ga_len;
		tv = STACK_TV_BOT(0);
		mnv_setenv_ext(iptr->isn_arg.string, tv_get_string(tv));
		clear_tv(tv);
		break;
	    ...
	    case ISN_STOREV:
		--ectx->ec_stack.ga_len;
		if (set_mnv_var_tv(iptr->isn_arg.number, STACK_TV_BOT(0))
								       == FAIL)
		...
		switch (iptr->isn_type)
		{
			...
			if (iptr->isn_arg.string == NULL)
			    ...
			    tv->vval.v_string =
					     mnv_strsave(iptr->isn_arg.string);
			break;
		    case ISN_PUSHCHANNEL:
			...
			tv->vval.v_string = iptr->isn_arg.string == NULL
				    ? NULL : mnv_strsave(iptr->isn_arg.string);
		}
		break;
		...
		break;
	    case ISN_UNLETENV:
		mnv_unsetenv_ext(iptr->isn_arg.unlet.ul_name);
		break;
...
/*
 * Call a "def" function from old MNV script.
 * Return OK or FAIL.
 */
    int
call_def_function(
...
#define STACK_TV_VAR(idx) (((typval_T *)ectx.ec_stack.ga_data) + ectx.ec_frame_idx + STACK_FRAME_SIZE + idx)
    ...
    // Commands behave like mnv9script.
...
failed_early:
    ...
    mnv_free(ectx.ec_stack.ga_data);
    mnv_free(ectx.ec_trystack.ga_data);
    if (ectx.ec_outer_ref != NULL)
    {
	if (ectx.ec_outer_ref->or_outer_allocated)
	    mnv_free(ectx.ec_outer_ref->or_outer);
...
printable_loopvarinfo(loopvarinfo_T *lvi)
{
	...
	if (lvi->lvi_loop[depth].var_idx == 0)
	    ...
	    mnv_snprintf((char *)ga.ga_data + ga.ga_len, 50, " $%d-$%d",
...
list_instructions(char *pfx, isn_T *instr, int instr_count, ufunc_T *ufunc)
{
	...
	switch (iptr->isn_type)
	{
		...
		smsg("%s%4d LOADV v:%s", pfx, current,
				       get_mnv_var_name(iptr->isn_arg.number));
		break;
	    case ISN_LOADSCRIPT:
		{
		    scriptref_T	    *sref = iptr->isn_arg.script.scriptref;
