...
/*
 * mnv9expr.c: Dealing with compiled function expressions
 */
	...
	imported_T *import) // found imported item, can be NULL
{
	...
	generate_MNV9SCRIPT(cctx, ISN_LOADSCRIPT,
	...
	*p = NUL;
	...
	if (si->sn_import_autoload && si->sn_state == SN_STATE_NOT_LOADED)
	    // "import autoload './dir/script.mnv'" or
	    // "import autoload './autoload/script.mnv'" - load script first
	...
	*end = p;
    ...
    // MNV9 script (variable is not in sn_var_vals): old style script.
	...
	int	error)
{
	...
	*(*arg + namelen) = NUL;
	...
	else
	{
	    ...
	    name = mnv_strnsave(*arg + 2, end - (*arg + 2));
	    if (name == NULL)
		goto theend;
		...
		case 'v': res = generate_LOADV(cctx, name);
			  break;
		case 's': if (current_script_is_mnv9())
			  {
			      semsg(_(e_cannot_use_s_colon_in_mnv9_script_str),
									 *arg);
			      goto theend;
			  }
			  if (is_expr && find_func(name, FALSE) != NULL)
			      ...
			      res = compile_load_scriptvar(cctx, name,
							    NULL, &end, NULL);
			  break;
		case 'g': if (mnv_strchr(name, AUTOLOAD_CHAR) == NULL)
			  {
	...
	else if (mnv_strchr(name, AUTOLOAD_CHAR) != NULL)
	{
	...
	else
	{
		...
		// already exists in a MNV9 script or when it's imported.
	...
	int	    argcount_init)
{
    ...
    mnv_strncpy(namebuf, *arg, varlen);
	...
	*arg += varlen + 1;
    ...
    is_autoload = mnv_strchr(name, AUTOLOAD_CHAR) != NULL;
	...
	if (idx >= 0)
	{
	    ...
	    {
		emsg(_(e_cannot_use_flatten_in_mnv9_script));
		goto theend;
	    }
...
#define MNV9_NAMESPACE_CHAR	(char_u *)"bgstvw"
...
to_name_end(char_u *arg, int use_namespace)
{
	...
	if (*p == ':' && (p != arg + 1
			     || !use_namespace
			     || mnv_strchr(MNV9_NAMESPACE_CHAR, *arg) == NULL))
	    break;
    return p;
}
...
/*
 * Get a lambda and compile it.  Uses MNV9 syntax.
 */
    int
...
compile_dict(char_u **arg, cctx_T *cctx, ppconst_T *ppconst)
{
	...
	if (**arg == '[')
	{
	    ...
	    *arg = skipwhite(*arg + 1);
		...
		mnv_snprintf(buf, NUMBUFLEN, "%lld", isn->isn_arg.number);
		isn->isn_type = ISN_PUSHS;
		isn->isn_arg.string = mnv_strsave((char_u *)buf);
	    }
	    if (isn->isn_type == ISN_PUSHS)
    ...
    *arg = *arg + 1;
    ...
    p = skipwhite(*arg);
    if (MNV_ISWHITE(**arg) && mnv9_comment_start(p))
	*arg += STRLEN(*arg);
...
compile_get_option(char_u **arg, cctx_T *cctx)
{
	...
	char_u	*name = mnv_strnsave(start, *arg - start);
 ...
 */
    static void
get_mnv_constant(char_u **arg, typval_T *rettv)
{
...
compile_leader(cctx_T *cctx, int numeric_only, char_u *start, char_u **end)
{
	...
	else
	{
	    ...
	    while (p > start && (p[-1] == '!' || MNV_ISWHITE(p[-1])))
	    {
		if (p[-1] == '!')
		    invert = !invert;
		--p;
	...
	ppconst_T *ppconst)
{
	...
	/*
	 * Constant MNV variable.
	 */
	case 'v':   get_mnv_constant(arg, rettv);
		    ret = NOTDONE;
		    break;
