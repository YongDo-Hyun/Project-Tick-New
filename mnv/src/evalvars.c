...
/*
 * Old MNV variables such as "v:version" are also available without the "v:".
...
typedef struct mnvvar mnvvar_T;

static struct mnvvar
{
...
{
    // The order here must match the VV_ defines in mnv.h!
    ...
    {VV_NAME("mnv_did_enter",	 VAR_NUMBER), NULL, VV_RO},
    ...
    {VV_NAME("mnv_did_init",	 VAR_NUMBER), NULL, VV_RO},
...
static dictitem_T	mnvvars_var;		// variable used for v:
static dict_T		mnvvardict;		// Dictionary with v: variables
#define mnvvarht  mnvvardict.dv_hashtab
...
/*
 * Initialize global and mnv special variables
 */
    void
evalvars_init(void)
{
    ...
    init_var_dict(&mnvvardict, &mnvvars_var, VAR_SCOPE);
    mnvvardict.dv_lock = VAR_FIXED;
	...
	if (p->vv_tv_type != VAR_UNKNOWN)
	    hash_add(&mnvvarht, p->vv_di.di_key, "initialization");
	if (p->vv_flags & VV_COMPAT)
	    // add to compat scope dict
	    hash_add(&compat_hashtab, p->vv_di.di_key, "initialization");
    }
    set_mnv_var_nr(VV_VERSION, MNV_VERSION_100);
    set_mnv_var_nr(VV_VERSIONLONG, MNV_VERSION_100 * 10000 + highest_patch());

    set_mnv_var_nr(VV_SEARCHFORWARD, 1L);
    set_mnv_var_nr(VV_HLSEARCH, 1L);
    set_mnv_var_nr(VV_EXITING, VVAL_NULL);
    set_mnv_var_dict(VV_COMPLETED_ITEM, dict_alloc_lock(VAR_FIXED));
    set_mnv_var_list(VV_ERRORS, list_alloc());
    set_mnv_var_dict(VV_EVENT, dict_alloc_lock(VAR_FIXED));

    set_mnv_var_nr(VV_FALSE, VVAL_FALSE);
    set_mnv_var_nr(VV_TRUE, VVAL_TRUE);
    set_mnv_var_nr(VV_NONE, VVAL_NONE);
    set_mnv_var_nr(VV_NULL, VVAL_NULL);
    set_mnv_var_nr(VV_NUMBERMAX, VARNUM_MAX);
    set_mnv_var_nr(VV_NUMBERMIN, VARNUM_MIN);
    set_mnv_var_nr(VV_NUMBERSIZE, sizeof(varnumber_T) * 8);
    set_mnv_var_nr(VV_SIZEOFINT, sizeof(int));
    set_mnv_var_nr(VV_SIZEOFLONG, sizeof(long));
    set_mnv_var_nr(VV_SIZEOFPOINTER, sizeof(char *));
    set_mnv_var_nr(VV_MAXCOL, MAXCOL);

    set_mnv_var_nr(VV_TYPE_NUMBER,  VAR_TYPE_NUMBER);
    set_mnv_var_nr(VV_TYPE_STRING,  VAR_TYPE_STRING);
    set_mnv_var_nr(VV_TYPE_FUNC,    VAR_TYPE_FUNC);
    set_mnv_var_nr(VV_TYPE_LIST,    VAR_TYPE_LIST);
    set_mnv_var_nr(VV_TYPE_DICT,    VAR_TYPE_DICT);
    set_mnv_var_nr(VV_TYPE_FLOAT,   VAR_TYPE_FLOAT);
    set_mnv_var_nr(VV_TYPE_BOOL,    VAR_TYPE_BOOL);
    set_mnv_var_nr(VV_TYPE_NONE,    VAR_TYPE_NONE);
    set_mnv_var_nr(VV_TYPE_JOB,     VAR_TYPE_JOB);
    set_mnv_var_nr(VV_TYPE_CHANNEL, VAR_TYPE_CHANNEL);
    set_mnv_var_nr(VV_TYPE_BLOB,    VAR_TYPE_BLOB);
    set_mnv_var_nr(VV_TYPE_CLASS,   VAR_TYPE_CLASS);
    set_mnv_var_nr(VV_TYPE_OBJECT,  VAR_TYPE_OBJECT);
    set_mnv_var_nr(VV_TYPE_TYPEALIAS,  VAR_TYPE_TYPEALIAS);
    set_mnv_var_nr(VV_TYPE_ENUM,    VAR_TYPE_ENUM);
    set_mnv_var_nr(VV_TYPE_ENUMVALUE,  VAR_TYPE_ENUMVALUE);
    set_mnv_var_nr(VV_TYPE_TUPLE,   VAR_TYPE_TUPLE);

    set_mnv_var_nr(VV_ECHOSPACE,    sc_col - 1);

    set_mnv_var_dict(VV_COLORNAMES, dict_alloc());

#ifdef FEAT_PYTHON3
    set_mnv_var_nr(VV_PYTHON3_VERSION, python3_version());
#endif
...
#if defined(EXITFREE)
/*
 * Free all mnv variables information on exit
 */
    void
evalvars_clear(void)
{
    ...
    hash_clear(&mnvvarht);
    hash_init(&mnvvarht);  // garbage_collect() will access it
...
garbage_collect_globvars(int copyID)
{
    ...
    int
garbage_collect_mnvvars(int copyID)
{
    return set_ref_in_ht(&mnvvarht, copyID, NULL, NULL);
}
    ...
    char_u	*fname_to)
{
    ...
    set_mnv_var_string(VV_CC_FROM, enc_from, -1);
    set_mnv_var_string(VV_CC_TO, enc_to, -1);
    set_mnv_var_string(VV_FNAME_IN, fname_from, -1);
    set_mnv_var_string(VV_FNAME_OUT, fname_to, -1);
    ...
    set_mnv_var_string(VV_CC_FROM, NULL, -1);
    set_mnv_var_string(VV_CC_TO, NULL, -1);
    set_mnv_var_string(VV_FNAME_IN, NULL, -1);
    set_mnv_var_string(VV_FNAME_OUT, NULL, -1);
...
eval_printexpr(char_u *fname, char_u *args)
{
    ...
    set_mnv_var_string(VV_FNAME_IN, fname, -1);
    set_mnv_var_string(VV_CMDARG, args, -1);
    ...
    set_mnv_var_string(VV_FNAME_IN, NULL, -1);
    set_mnv_var_string(VV_CMDARG, NULL, -1);
    current_sctx = saved_sctx;
    ...
    char_u	*outfile)
{
    ...
    set_mnv_var_string(VV_FNAME_IN, origfile, -1);
    set_mnv_var_string(VV_FNAME_NEW, newfile, -1);
    set_mnv_var_string(VV_FNAME_OUT, outfile, -1);
    ...
    set_mnv_var_string(VV_FNAME_IN, NULL, -1);
    set_mnv_var_string(VV_FNAME_NEW, NULL, -1);
    set_mnv_var_string(VV_FNAME_OUT, NULL, -1);
    current_sctx = saved_sctx;
}
    ...
    char_u	*outfile)
{
    ...
    set_mnv_var_string(VV_FNAME_IN, origfile, -1);
    set_mnv_var_string(VV_FNAME_DIFF, difffile, -1);
    set_mnv_var_string(VV_FNAME_OUT, outfile, -1);
    ...
    set_mnv_var_string(VV_FNAME_IN, NULL, -1);
    set_mnv_var_string(VV_FNAME_DIFF, NULL, -1);
    set_mnv_var_string(VV_FNAME_OUT, NULL, -1);
    current_sctx = saved_sctx;
}
#endif
...
eval_spell_expr(char_u *badword, char_u *expr)
{
    ...
    set_mnv_var_string(VV_VAL, badword, -1);
    if (p_verbose == 0)
	++emsg_off;
 ...
 */
    void
prepare_mnvvar(int idx, typval_T *save_tv)
{
    *save_tv = mnvvars[idx].vv_tv;
    mnvvars[idx].vv_str = NULL;  // don't free it now
    if (mnvvars[idx].vv_tv_type == VAR_UNKNOWN)
	hash_add(&mnvvarht, mnvvars[idx].vv_di.di_key, "prepare mnvvar");
}
 ...
 */
    void
restore_mnvvar(int idx, typval_T *save_tv)
{
    hashitem_T	*hi;

    mnvvars[idx].vv_tv = *save_tv;
    if (mnvvars[idx].vv_tv_type != VAR_UNKNOWN)
	return;

    hi = hash_find(&mnvvarht, mnvvars[idx].vv_di.di_key);
    if (HASHITEM_EMPTY(hi))
	internal_error("restore_mnvvar()");
    else
	hash_remove(&mnvvarht, hi, "restore mnvvar");
}
 ...
 */
    static void
list_mnv_vars(int *first)
{
    list_hashtable_vars(&mnvvarht, "v:", FALSE, first);
}
...
{
    return mnv_strchr((char_u *)"gwbt", name[0]) != NULL
	&& name[1] == ':'
	&& eval_isnamec(name[2]);
}

/*
 * Evaluate one MNV expression {expr} in string "p" and append the
...
/*
 * Evaluate all the MNV expressions {expr} in "str" and return the resulting
 ...
 *
 * When compiling a heredoc assignment to a variable in a MNV9 def function,
 * "mnv9compile" is set to TRUE. In this case, instead of generating a list of
 * string values from the heredoc, mnv9 instructions are generated.  On success
 * the returned list will be empty.
 *
 * Returns a List with {lines} or NULL on failure.
 */
    list_T *
heredoc_get(exarg_T *eap, char_u *cmd, int script_get, int mnv9compile)
{
    char_u	*theline = NULL;
    char_u	*marker;
    list_T	*l;
    ...
    cctx_T	*cctx = mnv9compile ? eap->cookie : NULL;
    int		count = 0;
	...
	*p = NUL;
	if (!script_get && mnv_islower(*marker))
	{
	...
	if (text_indent_len == -1 && *theline != NUL)
	{
	    ...
	    text_indent = mnv_strnsave(theline, text_indent_len);
	}
    ...
    if (mnv9compile && cctx->ctx_skip != SKIP_YES && !eval_failed)
...
ex_var(exarg_T *eap)
{
    ...
    if (!in_mnv9script())
    {
	semsg(_(e_str_cannot_be_used_in_legacy_mnv_script), ":var");
	return;
    }
 ...
 * ":let var =<< ..."		heredoc
 * ":let var: string"		MNV9 declaration
 *
...
ex_let(exarg_T *eap)
{
    ...
    if (eap->cmdidx == CMD_final && !mnv9script)
    {
	// In legacy MNV script ":final" is short for ":finally".
	ex_finally(eap);
	return;
    }
    if (eap->cmdidx == CMD_let && mnv9script)
    {
	emsg(_(e_cannot_use_let_in_mnv9_script));
	return;
    }
    ...
    // MNV9 assignment without ":let", ":const" or ":final"
    if (eap->arg == eap->cmd)
	...
	&& ((expr[1] == '=' && in_old_script(2))
		|| (expr[1] == '.' && expr[2] == '='));
    has_assign =  *expr == '=' || (mnv_strchr((char_u *)"+-*/%", *expr) != NULL
							    && expr[1] == '=');
	...
	else if (!ends_excmd2(eap->cmd, arg))
	{
		...
		if (!ends_excmd2(eap->cmd, skipwhite(argend)))
		    ...
		    // MNV9 declaration ":var name: type"
		    arg = mnv9_declare_scriptvar(eap, arg);
	    }
	    else
	    {
	...
	if (mnv9script && !eap->skip && (!MNV_ISWHITE(expr[-1])
    ...
    if (*expr != '=')
    {
	if (mnv9script && (flags & ASSIGN_NO_DECL) == 0)
	{
	    // +=, /=, etc. require an existing variable
	    semsg(_(e_cannot_use_operator_on_new_variable_str), eap->arg);
	}
	else if (mnv_strchr((char_u *)"+-*/%.", *expr) != NULL)
	{
	    op[0] = *expr;   // +=, -=, *=, /=, %= or .=
	    ++len;
	    if (expr[0] == '.' && expr[1] == '.') // ..=
    ...
    if (mnv9script && !eap->skip && (!MNV_ISWHITE(*argend)
					       || !IS_WHITE_OR_NUL(*expr)))
    {
	mnv_strncpy(op, expr - len, len);
	semsg(_(e_white_space_required_before_and_after_str_at_str),
							       op, argend);
    }
 ...
 * l[idx].
 * In MNV9 script also skip over ": type" if "include_type" is TRUE.
 */
    char_u *
skip_var_one(char_u *arg, int include_type)
{
    ...
    // Same for "s: type".
    if (mnv9 && end == arg + 2 && end[-1] == ':')
	--end;

    if (include_type && mnv9)
    {
	if (*skipwhite(end) == ':')
    ...
    int		*first)
{
	...
	if (!HASHITEM_EMPTY(hi))
	{
	    ...
	    mnv_strncpy((char_u *)buf, (char_u *)prefix, IOSIZE - 1);
	    mnv_strcat((char_u *)buf, di->di_key, IOSIZE);
...
list_arg_vars(exarg_T *eap, char_u *arg, int *first)
{
	...
	if (error || eap->skip)
	{
	    ...
	    if (!MNV_ISWHITE(*arg) && !ends_excmd(*arg))
	    {
		emsg_severe = TRUE;
		if (!did_emsg)
	...
	else
	{
		...
		else
		{
			...
			if (arg == arg_subsc && len == 2 && name[1] == ':')
			{
				...
				case 'v': list_mnv_vars(first); break;
    ...
    char_u	*op)
{
    ...
    {
	if (op != NULL && mnv_strchr((char_u *)"+-*/%", *op) != NULL)
	    semsg(_(e_wrong_variable_type_for_str_equal), op);
	else if (endchars != NULL
			      && mnv_strchr(endchars, *skipwhite(arg)) == NULL)
    ...
    *p = NUL;
    ...
    if ((opt_type == gov_bool
		...
		|| opt_type == gov_hidden_number)
		     && (tv->v_type != VAR_STRING || !in_mnv9script()))
    {
    ...
    // In MNV9 script also don't convert a number to string.
    else if (tv->v_type != VAR_BOOL && tv->v_type != VAR_SPECIAL
		     && (!in_mnv9script() || tv->v_type != VAR_NUMBER))
    {
	...
	if (opt_type == gov_number
				 || (opt_type == gov_bool && !in_mnv9script()))
	{
	    switch (*op)
	    {
    ...
    char_u	*op)
{
	...
	semsg(_(e_wrong_variable_type_for_str_equal), op);
			  ...
			  && mnv_strchr(endchars, *skipwhite(arg + 1)) == NULL)
	emsg(_(e_unexpected_characters_in_let));
    else
    {
	char_u	*ptofree = NULL;
    ...
    int		var_idx)	// variable index for "let [a, b] = list"
{
    ...
    if (in_mnv9script() && (flags & (ASSIGN_NO_DECL | ASSIGN_DECL)) == 0
			&& (flags & (ASSIGN_CONST | ASSIGN_FINAL)) == 0
				  && mnv_strchr((char_u *)"$@&", *arg) != NULL)
    {
	mnv9_declare_error(arg);
	return NULL;
    }
	...
	{
	    if (endchars != NULL && mnv_strchr(endchars,
 ...
 * Also used for MNV9 script.  "callback" is invoked as:
    ...
    void	*cookie)
{
	...
	else
	{
	    ...
	    if (name_end == NULL || (!MNV_ISWHITE(*name_end)
						    && !ends_excmd(*name_end)))
	    {
		if (name_end != NULL)
		{
...
do_unlet(char_u *name, int forceit)
{
    ...
    // can't :unlet a script variable in MNV9 script
    if (in_mnv9script() && check_mnv9_unlet(name) == FAIL)
	return FAIL;

    ht = find_var_ht(name, &varname);

    // can't :unlet a script variable in MNV9 script from a function
    if (ht == get_script_local_ht()
	    && SCRIPT_ID_VALID(current_sctx.sc_sid)
	    && SCRIPT_ITEM(current_sctx.sc_sid)->sn_version
							 == SCRIPT_VERSION_MNV9
	    && check_mnv9_unlet(name) == FAIL)
	return FAIL;
...
report_lockvar_member(char *msg, lval_T *lp)
{
	...
	{
	    vname = mnv_strnsave(lp->ll_name, lp->ll_name_end - lp->ll_name);
	    if (vname == NULL)
		return;
	    did_alloc = TRUE;
	}
...
get_user_var_name(expand_T *xp, int idx)
{
    ...
    // v: variables
    if (vidx < VV_LEN)
	return cat_prefix_varname('v', (char_u *)mnvvars[vidx++].vv_name);

    MNV_CLEAR(varnamebuf);
    varnamebuflen = 0;
    return NULL;
}
...
get_var_special_name(int nr)
{
    switch (nr)
    {
	case VVAL_FALSE: return in_mnv9script() ? "false" : "v:false";
	case VVAL_TRUE:  return in_mnv9script() ? "true" : "v:true";
	case VVAL_NULL:  return in_mnv9script() ? "null" : "v:null";
 ...
 */
    int
find_mnv_var(char_u *name, int *di_flags)
{
    dictitem_T	    *di = find_var_in_ht(&mnvvarht, 0, name, TRUE);
    struct mnvvar   *vv;

    if (di == NULL)
	return -1;
    *di_flags = di->di_flags;
    vv = (struct mnvvar *)((char *)di - offsetof(mnvvar_T, vv_di));
    return (int)(vv - mnvvars);
}
 ...
 */
    void
set_mnv_var_type(int idx, vartype_T type)
{
    mnvvars[idx].vv_tv_type = type;
}

/*
 * Set number v: variable to "val".
 * Note that this does not set the type, use set_mnv_var_type() for that.
 */
    void
set_mnv_var_nr(int idx, varnumber_T val)
{
    mnvvars[idx].vv_nr = val;
}

    char *
get_mnv_var_name(int idx)
{
    return mnvvars[idx].vv_name;
}
 ...
 */
    typval_T *
get_mnv_var_tv(int idx)
{
    return &mnvvars[idx].vv_tv;
}

    type_T *
get_mnv_var_type(int idx, garray_T *type_list)
{
    if (mnvvars[idx].vv_type != NULL)
	return mnvvars[idx].vv_type;
    return typval2type_mnvvar(&mnvvars[idx].vv_tv, type_list);
}
 ...
 */
    int
set_mnv_var_tv(int idx, typval_T *tv)
{
    if (mnvvars[idx].vv_tv_type != tv->v_type)
    {
    ...
    if (mnvvars[idx].vv_flags & VV_RO)
    {
	semsg(_(e_cannot_change_readonly_variable_str), mnvvars[idx].vv_name);
	return FAIL;
    }
    if (sandbox && (mnvvars[idx].vv_flags & VV_RO_SBX))
    {
	semsg(_(e_cannot_set_variable_in_sandbox_str), mnvvars[idx].vv_name);
	return FAIL;
    }
    clear_tv(&mnvvars[idx].vv_di.di_tv);
    mnvvars[idx].vv_di.di_tv = *tv;
    return OK;
}
 ...
 */
    varnumber_T
get_mnv_var_nr(int idx)
{
    return mnvvars[idx].vv_nr;
}
 ...
 */
    char_u *
get_mnv_var_str(int idx)
{
    return tv_get_string(&mnvvars[idx].vv_tv);
}
 ...
 */
    list_T *
get_mnv_var_list(int idx)
{
    return mnvvars[idx].vv_list;
}
 ...
 */
    dict_T *
get_mnv_var_dict(int idx)
{
    return mnvvars[idx].vv_dict;
}
...
set_mnv_var_char(int c)
{
    ...
    set_mnv_var_string(VV_CHAR, buf, (int)buflen);
}
...
{
    if (set_prevcount)
	mnvvars[VV_PREVCOUNT].vv_nr = mnvvars[VV_COUNT].vv_nr;
    mnvvars[VV_COUNT].vv_nr = count;
    mnvvars[VV_COUNT1].vv_nr = count1;
}
 ...
 */
    void
save_mnvvars(mnvvars_save_T *vvsave)
{
    vvsave->vv_prevcount = mnvvars[VV_PREVCOUNT].vv_nr;
    vvsave->vv_count = mnvvars[VV_COUNT].vv_nr;
    vvsave->vv_count1 = mnvvars[VV_COUNT1].vv_nr;
}

/*
 * Restore variables saved by save_mnvvars().
 */
    void
restore_mnvvars(mnvvars_save_T *vvsave)
{
    mnvvars[VV_PREVCOUNT].vv_nr = vvsave->vv_prevcount;
    mnvvars[VV_COUNT].vv_nr = vvsave->vv_count;
    mnvvars[VV_COUNT1].vv_nr = vvsave->vv_count1;
}
...
{
    clear_tv(&mnvvars[idx].vv_di.di_tv);
    mnvvars[idx].vv_tv_type = VAR_STRING;
    if (val == NULL)
	mnvvars[idx].vv_str = NULL;
    else if (len == -1)
	mnvvars[idx].vv_str = mnv_strsave(val);
    else
	mnvvars[idx].vv_str = mnv_strnsave(val, len);
}

    void
set_mnv_var_string_direct(
    int		idx,
    char_u	*val)
{
    clear_tv(&mnvvars[idx].vv_di.di_tv);
 ...
 */
    void
set_mnv_var_list(int idx, list_T *val)
{
    clear_tv(&mnvvars[idx].vv_di.di_tv);
    mnvvars[idx].vv_tv_type = VAR_LIST;
    mnvvars[idx].vv_list = val;
    if (val != NULL)
	++val->lv_refcount;
}
 ...
 */
    void
set_mnv_var_dict(int idx, dict_T *val)
{
    clear_tv(&mnvvars[idx].vv_di.di_tv);
    mnvvars[idx].vv_tv_type = VAR_DICT;
    mnvvars[idx].vv_dict = val;
    if (val == NULL)
	return;
...
set_reg_var(int c)
{
    ...
    if (mnvvars[VV_REG].vv_str == NULL || mnvvars[VV_REG].vv_str[0] != c)
	set_mnv_var_string(VV_REG, regname, 1);
}
...
{
    if (oldval == NULL)
	return mnvvars[VV_EXCEPTION].vv_str;

    mnvvars[VV_EXCEPTION].vv_str = oldval;
    return NULL;
}
...
{
    if (oldval == NULL)
	return mnvvars[VV_THROWPOINT].vv_str;

    mnvvars[VV_THROWPOINT].vv_str = oldval;
    return NULL;
}
...
set_cmdarg(exarg_T *eap, char_u *oldarg)
{
    ...
    oldval = mnvvars[VV_CMDARG].vv_str;
    if (eap == NULL)
    {
	mnv_free(oldval);
	mnvvars[VV_CMDARG].vv_str = oldarg;
	return NULL;
    }
	...
	*newval = NUL;
    ...
    mnvvars[VV_CMDARG].vv_str = newval;
    return oldval;
}
    ...
    int		flags)		// EVAL_VAR_ flags
{
    ...
    if (tv == NULL && (in_mnv9script() || STRNCMP(name, "s:", 2) == 0))
    {
	...
	else if (in_mnv9script() && (flags & EVAL_VAR_NO_FUNC) == 0)
	{
	    ...
	    // In MNV9 script we can get a function reference by using the
		...
		if (rettv != NULL)
		{
			...
			rettv->vval.v_string = mnv_strnsave(ufunc->uf_name, ufunc->uf_namelen);
...
find_var(char_u *name, hashtab_T **htp, int no_autoload)
{
	...
	*htp = ht;
    ...
    // in MNV9 script items without a scope can be script-local
    if (in_mnv9script() && name[0] != NUL && name[1] != ':')
    {
							    ...
							    char_u **namep)
{
    hashtab_T	*ht;
    dictitem_T	*ret = NULL;
    // When using "mnv9script autoload" script-local items are prefixed but can
    // be used with s:name.
    int check_sid = sid > 0 ? sid : current_sctx.sc_sid;
    if (SCRIPT_ID_VALID(check_sid)
		   && (in_mnv9script() || (name[0] == 's' && name[1] == ':')))
    {
	...
	cctx_T	*dummy UNUSED)
{
	...
	mnv_strncpy(buffer, name, len);
	p = buffer;
    }
    else
    {
	p = mnv_strnsave(name, len);
	if (p == NULL)
	    return FAIL;
    }
...
find_var_ht(char_u *name, char_u **varname)
{
	...
	*varname = name;
	...
	// In MNV9 script items at the script level are script-local, except
	// for autoload names.
	if (in_mnv9script() && mnv_strchr(name, AUTOLOAD_CHAR) == NULL)
	{
	    ht = get_script_local_ht();
	    if (ht != NULL)
		return ht;
    ...
    *varname = name + 2;
    ...
    if (mnv_strchr(name + 2, ':') != NULL
			       || mnv_strchr(name + 2, AUTOLOAD_CHAR) != NULL)
	return NULL;
    ...
    int		*type_error)
{
    ...
    {
	MNV_CLEAR(di->di_tv.vval.v_string);
	if (copy || tv->v_type != VAR_STRING)
	{
		...
		di->di_tv.vval.v_string = mnv_strsave(val);
	}
	else
	{
    ...
    int		var_idx)    // index for ":let [a, b] = list"
{
	...
	if (in_mnv9script()
		...
		|| find_var_autoload_prefix(name, 0, NULL, &auto_name)
								    != NULL))
	{
	    // In a mnv9 autoload script an exported variable is put in the
    ...
    var_in_mnv9script = is_script_local && current_script_is_mnv9();
    if (var_in_mnv9script && name[0] == '_' && name[1] == NUL)
    {
	...
	if (!in_mnv9script())
	{
	    semsg(_(e_cannot_create_mnv9_script_variable_in_function_str),
									 name);
	    goto failed;
	}
    }
	...
	if ((di->di_flags & DI_FLAGS_RELOAD) == 0)
	{
	    ...
	    if (var_in_mnv9script && (flags & ASSIGN_FOR_LOOP) == 0
	...
	else
	{
	    ...
	    // A MNV9 script-local variable is also present in sn_all_vars
	    // and sn_var_vals.  It may set "type" from "tv".
	    if (var_in_mnv9script || var_in_autoload)
		update_mnv9_script_var(FALSE, di,
	...
	// add a new variable
	if (var_in_mnv9script && (flags & ASSIGN_NO_DECL))
	{
	...
	if (ht == &mnvvarht || ht == get_funccal_args_ht())
	{
	    semsg(_(e_illegal_variable_name_str), name);
	    goto failed;
	}

	// Make sure the variable name is valid.  In MNV9 script an
	// autoload variable must be prefixed with "g:" unless in an
	// autoload script.
	if (!valid_varname(varname, -1, !mnv9script
	...
	// A MNV9 script-local variable is also added to sn_all_vars and
	// sn_var_vals. It may set "type" from "tv".
	if (var_in_mnv9script || var_in_autoload)
	    update_mnv9_script_var(TRUE, di,
		    var_in_autoload ? name : di->di_key, flags,
			  tv, &type, (flags & ASSIGN_NO_MEMBER_TYPE) == 0);
    }
...
{
    // Allow for w: b: s: and t:.  In MNV9 script s: is not allowed, because
    // the name can be used without the s: prefix.
    // Allow autoload variable.
    if (!((mnv_strchr((char_u *)"wbt", name[0]) != NULL
		    || (!in_mnv9script() && name[0] == 's')) && name[1] == ':')
	    && !ASCII_ISUPPER((name[0] != NUL && name[1] == ':')
						     ? name[2] : name[0])
	    && mnv_strchr(name, '#') == NULL)
    {
...
valid_varname(char_u *varname, int len, int autoload)
{
	...
	if (!eval_isnamec1(*p) && (p == varname || !MNV_ISDIGIT(*p))
...
set_option_from_tv(char_u *varname, typval_T *varp)
{
	...
	if ((opt_p_flags & (P_NUM|P_BOOL))
		&& (!in_mnv9script() || varp->v_type != VAR_STRING))
...
{
    set_mnv_var_string(VV_OPTION_NEW, NULL, -1);
    set_mnv_var_string(VV_OPTION_OLD, NULL, -1);
    set_mnv_var_string(VV_OPTION_OLDLOCAL, NULL, -1);
    set_mnv_var_string(VV_OPTION_OLDGLOBAL, NULL, -1);
    set_mnv_var_string(VV_OPTION_TYPE, NULL, -1);
    set_mnv_var_string(VV_OPTION_COMMAND, NULL, -1);
}
...
{
    struct mnvvar   *vp = &mnvvars[VV_ERRORS];

    if (vp->vv_tv_type != VAR_LIST || mnvvars[VV_ERRORS].vv_list == NULL)
	// Make sure v:errors is a list.
	set_mnv_var_list(VV_ERRORS, list_alloc());
    list_append_string(mnvvars[VV_ERRORS].vv_list, gap->ga_data, gap->ga_len);
}
...
var_redir_start(char_u *name, int append)
{
    ...
    redir_varname = mnv_strsave(name);
...
var_redir_stop(void)
{
	...
	MNV_CLEAR(redir_ga.ga_data);

	MNV_CLEAR(redir_lval);
    }
    MNV_CLEAR(redir_varname);
}
...
put_callback(callback_T *cb, typval_T *tv)
{
    ...
    {
	tv->v_type = VAR_FUNC;
	tv->vval.v_string = mnv_strsave(cb->cb_name);
	func_ref(cb->cb_name);
    }
}
...
set_callback(callback_T *dest, callback_T *src)
{
    *dest = *src;
    ...
    {
	dest->cb_name = mnv_strsave(src->cb_name);
...
expand_autload_callback(callback_T *cb)
{
    ...
    if (!in_mnv9script() || cb->cb_name == NULL
	    || (!cb->cb_free_name
	...
	if (cb->cb_name == cb->cb_partial->pt_name)
	    cb->cb_name = newname;
	mnv_free(cb->cb_partial->pt_name);
