...
/*
 * mnv9script.c: :mnv9script, :import, :export and friends
 */

#include "mnv.h"

/*
 * Return TRUE when currently using MNV9 script syntax.
 * Does not go up the stack, a ":function" inside mnv9script uses legacy
 * syntax.
 */
    int
in_mnv9script(void)
...
/*
 * Return TRUE if the current script is MNV9 script.
 * This also returns TRUE in a legacy function in a MNV9 script.
 */
    int
current_script_is_mnv9(void)
{
...
#ifdef FEAT_EVAL
/*
 * Clear MNV9 script-local variables and functions.
 */
    void
clear_mnv9_scriptlocal_vars(int sid)
{
 ...
 */
    void
ex_mnv9script(exarg_T *eap UNUSED)
{
#ifdef FEAT_EVAL
    ...
    {
	emsg(_(e_mnv9script_must_be_first_command_in_script));
	return;
    }
    ...
    si->sn_version = SCRIPT_VERSION_MNV9;

    if (STRCMP(p_cpo, CPO_MNV) != 0)
    {
	si->sn_save_cpo = mnv_strsave(p_cpo);
	set_option_value_give_err((char_u *)"cpo",
					 0L, (char_u *)CPO_MNV, OPT_NO_REDRAW);
    }
#else
...
#if defined(FEAT_EVAL)
/*
 * When in MNV9 script give an error and return FAIL.
 */
    int
not_in_mnv9(exarg_T *eap)
{
    if (in_mnv9script())
	switch (eap->cmdidx)
	{
	    ...
	    case CMD_t:
	    case CMD_xit:
		semsg(_(e_command_not_supported_in_mnv9_script_missing_var_str), eap->cmd);
		return FAIL;
	    default: break;
	}
    return OK;
 ...
 */
    int
mnv9_comment_start(char_u *p)
{
#ifdef FEAT_EVAL
    return p[0] == '#' && !mnv9_bad_comment(p);
#else
    return p[0] == '#' && (p[1] != '{' || p[2] == '{');
#endif
}
...
ex_incdec(exarg_T *eap)
{
    ...
    if (eap->cmd == NULL)
	return;
    mnv_snprintf((char *)eap->cmd, len, "%s %c= 1 |", cmd + 2,
...
ex_export(exarg_T *eap UNUSED)
{
    ...
    emsg(_(e_export_can_only_be_used_in_mnv9script));
}
	...
	void	    *cctx)
{
    ...
    if (STRNCMP(arg, "autoload", 8) == 0 && MNV_ISWHITE(arg[8]))
    {
	is_autoload = TRUE;
	arg = skipwhite(arg + 8);
    }
	...
	{
	    // Relative to current script: "./name.mnv", "../../name.mnv".
	    len = STRLEN(si->sn_name) - STRLEN(tail) + STRLEN(tv.vval.v_string) + 2;
	    from_name = alloc((int)len);
	    if (from_name == NULL)
		goto erret;
	    mnv_strncpy(from_name, si->sn_name, tail - si->sn_name);
	    add_pathsep(from_name);
	    STRCAT(from_name, tv.vval.v_string);
	}
	else
	    from_name = mnv_strsave(tv.vval.v_string);
	...
	if (from_name == NULL)
	    goto erret;
	mnv_snprintf((char *)from_name, len, "autoload/%s", tv.vval.v_string);
	...
	if (from_name == NULL)
	    goto erret;
	mnv_snprintf((char *)from_name, len, "import/%s", tv.vval.v_string);
	...
	as_name = mnv_strnsave(p, arg - p);
	arg = skipwhite(arg);
    }
    else
    {
	char_u *p = gettail(tv.vval.v_string);
	char_u *end = (char_u *)strstr((char *)p, ".mnv");
	...
	{
	    semsg(_(e_imported_script_must_use_as_or_end_in_dot_mnv_str), p);
	    goto erret;
	}
	if (end == p)
	{
	    semsg(_(e_cannot_import_dot_mnv_without_using_as), p);
	    goto erret;
	}
	as_name = mnv_strnsave(p, end - p);
    }
 ...
 */
    char_u *
mnv9_declare_scriptvar(exarg_T *eap, char_u *arg)
{
    char_u	    *p;
    char_u	    *name;
    scriptitem_T    *si = SCRIPT_ITEM(current_sctx.sc_sid);
...
/*
 * MNV9 part of adding a script variable: add it to sn_all_vars (lookup by name
...
find_typval_in_script(typval_T *dest, scid_T sid, int must_find)
{
    ...
    if (si->sn_version != SCRIPT_VERSION_MNV9)
