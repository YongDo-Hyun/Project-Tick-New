...
int mnv_lstat(const char *name, stat_T *stp);
int mnv_stat(const char *name, stat_T *stp);
...
char_u *serverGetMNVNames(void);
int serverSendReply(char_u *name, char_u *reply);
int serverSendToMNV(char_u *name, char_u *cmd, char_u **result, void *ptarget, int asExpr, int timeout, int silent);
...
UINT WINAPI mnvGetDpiForSystem(void);
