...
int get_menu_index(mnvmenu_T *menu, int state);
int menu_is_menubar(char_u *name);
int menu_is_popup(char_u *name);
int menu_is_child_of_popup(mnvmenu_T *menu);
int menu_is_toolbar(char_u *name);
int menu_is_separator(char_u *name);
int get_menu_mode_flag(void);
void show_popupmenu(void);
int check_menu_pointer(mnvmenu_T *root, mnvmenu_T *menu_to_check);
void gui_create_initial_menus(mnvmenu_T *menu);
void gui_update_menus(int modes);
int gui_is_menu_shortcut(int key);
void gui_mch_toggle_tearoffs(int enable);
void execute_menu(exarg_T *eap, mnvmenu_T *menu, int mode_idx);
void ex_emenu(exarg_T *eap);
void winbar_click(win_T *wp, int col);
mnvmenu_T *gui_find_menu(char_u *path_name);
