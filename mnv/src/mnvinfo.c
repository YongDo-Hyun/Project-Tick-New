...
/*
 * mnvinfo.c: mnvinfo related functions
 */
...
/*
 * Structure used for reading from the mnvinfo file.
 */
typedef struct
{
    char_u	*vir_line;	// text of the current line
    FILE	*vir_fd;	// file descriptor
    mnvconv_T	vir_conv;	// encoding conversion
    int		vir_version;	// mnvinfo version detected or -1
...
static int  mnvinfo_errcnt;

/*
 * Find the parameter represented by the given character (eg ''', ':', '"', or
 * '/') in the 'mnvinfo' option and return a pointer to the string after it.
 * Return NULL if the parameter is not specified in the string.
 */
    static char_u *
find_mnvinfo_parameter(int type)
{
    char_u  *p;
	...
	if (*p == 'n')		    // 'n' is always the last one
	    break;
	p = mnv_strchr(p, ',');	    // skip until next ','
...
/*
 * Find the parameter represented by the given character (eg ', :, ", or /),
 * and return its associated value in the 'mnvinfo' string.
 * Only works for number parameters, not for 'r' or 'n'.
 * If the parameter is not specified in the string or there is no following
 * number, return -1.
 */
    int
get_mnvinfo_parameter(int type)
{
    char_u  *p;

    p = find_mnvinfo_parameter(type);
...
/*
 * Get the mnvinfo file name to use.
 * If "file" is given and not empty, use it (has already been expanded by
 * cmdline functions).
 * Otherwise use "-i file_name", value from 'mnvinfo' or the default, and
 * expand environment variables.
 * Returns an allocated string.  NULL when out of memory.
 */
    static char_u *
mnvinfo_filename(char_u *file)
{
    if (file == NULL || *file == NUL)
    {
	size_t  len;

	if (*p_mnvinfofile != NUL)
	    file = p_mnvinfofile;
	else if ((file = find_mnvinfo_parameter('n')) == NULL || *file == NUL)
	{
#ifdef MNVINFO_FILE2
# ifdef VMS
...
# else
#  ifdef MSWIN
	    // Use $MNV only if $HOME is the default "C:/".
	    if (STRCMP(mnv_getenv((char_u *)"HOME", NULL), "C:/") == 0
...
# endif
	    {
		// don't use $MNV when not available.
		expand_env((char_u *)"$MNV", NameBuff, MAXPATHL);
		if (STRCMP("$MNV", NameBuff) != 0)  // $MNV was expanded
		    file = (char_u *)MNVINFO_FILE2;
		else
		    file = (char_u *)MNVINFO_FILE;
	    }
	    else
#endif
		file = (char_u *)MNVINFO_FILE;
	...
	return mnv_strnsave(file, len);
    }

    return mnv_strsave(file);
}

/*
 * write string to mnvinfo file
 ...
 */
    static void
mnvinfo_writestring(FILE *fd, char_u *p)
{
    int		c;
    char_u	*s;
    int		len = 0;
...
/*
 * Check string read from mnvinfo file.
    ...
    int		convert)	    // convert the string
{
    ...
    if (virp->vir_line[off] == Ctrl_V && mnv_isdigit(virp->vir_line[off + 1]))
    {
	...
	if (retval == NULL)
	{
	    ...
	    (void)mnv_fgets(virp->vir_line, 10, virp->vir_fd);
	    return NULL;
	}
	(void)mnv_fgets(retval, (int)len, virp->vir_fd);
	s = retval + 1;	    // Skip the leading '<'
    }
    else
    {
	retval = mnv_strsave(virp->vir_line + off);
	if (retval == NULL)
	    return NULL;
	s = retval;
    }
...
/*
 * Read a line from the mnvinfo file.
 * Returns TRUE for end-of-file;
 */
    static int
mnvinfo_readline(vir_T *virp)
{
    return mnv_fgets(virp->vir_line, LSIZE, virp->vir_fd);
}

    static int
read_mnvinfo_bufferlist(
    vir_T	*virp,
    int		writing)
{
    char_u	*tab;
    ...
    return mnvinfo_readline(virp);
}

/*
 * Return TRUE if "name" is on removable media (depending on 'mnvinfo').
 */
    static int
removable(char_u *name)
{
    ...
    static void
write_mnvinfo_bufferlist(FILE *fp)
{
    buf_T	*buf;
    win_T	*win;
    tabpage_T	*tp;
    ...
    if (find_mnvinfo_parameter('%') == NULL)
	return;

    // Without a number -1 is returned: do all buffers.
    max_buffers = get_mnvinfo_parameter('%');
...
#define LINE_BUF_LEN (MAXPATHL + 40)
	...
	mnv_snprintf_add((char *)line, LINE_BUF_LEN, "\t%ld\t%d",
			(long)buf->b_last_cursor.lnum,
			buf->b_last_cursor.col);
	mnvinfo_writestring(fp, line);
    }
    mnv_free(line);
}

/*
 * Buffers for history read from a mnvinfo file.  Only valid while reading.
 */
static histentry_T *mnvinfo_history[HIST_COUNT] =
					       {NULL, NULL, NULL, NULL, NULL};
static int	mnvinfo_hisidx[HIST_COUNT] = {0, 0, 0, 0, 0};
static int	mnvinfo_hislen[HIST_COUNT] = {0, 0, 0, 0, 0};
static int	mnvinfo_add_at_front = FALSE;
...
/*
 * Prepare for reading the history from the mnvinfo file.
 * This allocates history arrays to store the read history lines.
 */
    static void
prepare_mnvinfo_history(int asklen, int writing)
{
    int	    i;
    int	    num;
    int	    type;
    ...
    init_history();
    hislen = get_hislen();
    mnvinfo_add_at_front = (asklen != 0 && !writing);
    if (asklen > hislen)
	asklen = hislen;
	...
	// from mnvinfo previously are also considered empty.  If there are
	// more spaces available than we request, then fill them up.
	for (i = 0, num = 0; i < hislen; i++)
	    if (histentry[i].hisstr == NULL || histentry[i].mnvinfo)
		num++;
	len = asklen;
	if (num > len)
	    len = num;
	if (len <= 0)
	    mnvinfo_history[type] = NULL;
	else
	    mnvinfo_history[type] = LALLOC_MULT(histentry_T, len);
	if (mnvinfo_history[type] == NULL)
	    len = 0;
	mnvinfo_hislen[type] = len;
	mnvinfo_hisidx[type] = 0;
    }
}

/*
 * Accept a line from the mnvinfo, store it in the history array when it's
 * new.
 */
    static int
read_mnvinfo_history(vir_T *virp, int writing)
{
    int		type;
    long_u	len;
    char_u	*val = NULL;
    ...
    if (mnvinfo_hisidx[type] >= mnvinfo_hislen[type])
	goto done;

    val = mnvinfo_readstring(virp, 1, TRUE);
    if (val == NULL || *val == NUL)
	goto done;

    int sep = (*val == ' ' ? NUL : *val);

    if (in_history(type, val + (type == HIST_SEARCH), mnvinfo_add_at_front,
								sep, writing))
	goto done;
	...
	// Not a search entry: No separator in the mnvinfo
	// file, add a NUL separator.
	mch_memmove(p, val, (size_t)len + 1);	// +1 to include the NUL
	p[len + 1] = NUL;			// put the separator *after* the string's NUL
    }
    mnvinfo_history[type][mnvinfo_hisidx[type]].hisstr = p;
    mnvinfo_history[type][mnvinfo_hisidx[type]].hisstrlen = (size_t)len;
    mnvinfo_history[type][mnvinfo_hisidx[type]].time_set = 0;
    mnvinfo_history[type][mnvinfo_hisidx[type]].mnvinfo = TRUE;
    mnvinfo_history[type][mnvinfo_hisidx[type]].hisnum = 0;
    mnvinfo_hisidx[type]++;
...
/*
 * Accept a new style history line from the mnvinfo, store it in the history
 * array when it's new.
 */
    static void
handle_mnvinfo_history(
	garray_T    *values,
	int	    writing)
{
    int		type;
    ...
    if (in_history(type, val, mnvinfo_add_at_front, sep, writing))
	return;

    // If lines were written by an older MNV we need to avoid
    // getting duplicates. See if the entry already exists.
    for (idx = 0; idx < mnvinfo_hisidx[type]; ++idx)
    {
	p = mnvinfo_history[type][idx].hisstr;
	len = mnvinfo_history[type][idx].hisstrlen;
	if (STRCMP(val, p) == 0
    ...
    if (p != NULL)
    {
	mnvinfo_history[type][idx].time_set = vp[1].bv_nr;
	if (!overwrite)
	{
	    mch_memmove(p, val, (size_t)len + 1);
	    // Put the separator after the NUL.
	    p[len + 1] = sep;
	    mnvinfo_history[type][idx].hisstr = p;
	    mnvinfo_history[type][idx].hisstrlen = (size_t)len;
	    mnvinfo_history[type][idx].hisnum = 0;
	    mnvinfo_history[type][idx].mnvinfo = TRUE;
	    mnvinfo_hisidx[type]++;
	}
    }
}
...
/*
 * Concatenate history lines from mnvinfo after the lines typed in this MNV.
 */
    static void
concat_history(int type)
{
    ...
    idx = *hisidx + mnvinfo_hisidx[type];
    if (idx >= hislen)
	idx -= hislen;
    else if (idx < 0)
	idx = hislen - 1;
    if (mnvinfo_add_at_front)
	*hisidx = idx;
    else
    {
	if (*hisidx == -1)
	...
	do
	{
	    if (histentry[idx].hisstr != NULL || histentry[idx].mnvinfo)
		break;
	    if (++idx == hislen)
		idx = 0;
	} while (idx != *hisidx);
    ...
    for (i = 0; i < mnvinfo_hisidx[type]; i++)
    {
	mnv_free(histentry[idx].hisstr);
	histentry[idx].hisstr = mnvinfo_history[type][i].hisstr;
	histentry[idx].hisstrlen = mnvinfo_history[type][i].hisstrlen;
	histentry[idx].mnvinfo = TRUE;
	histentry[idx].time_set = mnvinfo_history[type][i].time_set;
	if (--idx < 0)
	    idx = hislen - 1;
    }
    idx += 1;
...
/*
 * Merge history lines from mnvinfo and lines typed in this MNV based on the
 * timestamp;
 */
    static void
merge_history(int type)
{
    ...
    max_len = hislen + mnvinfo_hisidx[type];
    ...
    for (i = 0; i < mnvinfo_hisidx[type]; i++)
	tot_hist[i] = &mnvinfo_history[type][i];
    len = i;
    for (i = 0; i < hislen; i++)
    ...
    *hisidx = (i < len ? i : len) - 1;
    ...
    for (i = 0; i < mnvinfo_hisidx[type]; i++)
    {
	mnv_free(mnvinfo_history[type][i].hisstr);
	mnvinfo_history[type][i].hisstrlen = 0;
    }
    for (i = 0; i < hislen; i++)
    {
	mnv_free(histentry[i].hisstr);
...
/*
 * Finish reading history lines from mnvinfo.  Not used when writing mnvinfo.
 */
    static void
finish_mnvinfo_history(vir_T *virp)
{
    int	type;
    int merge = virp->vir_version >= MNVINFO_VERSION_WITH_HISTORY;
	...
	MNV_CLEAR(mnvinfo_history[type]);
	mnvinfo_hisidx[type] = 0;
    }
}

/*
 * Write history to mnvinfo file in "fp".
 * When "merge" is TRUE merge history lines with a previously read mnvinfo
 * file, data is in mnvinfo_history[].
 * When "merge" is FALSE just write all history lines.  Used for ":wmnvinfo!".
 */
    static void
write_mnvinfo_history(FILE *fp, int merge)
{
    int	    i;
    int	    type;
    int	    num_saved;
	...
	num_saved = get_mnvinfo_parameter(hist_type2char(type, FALSE));
	if (num_saved == 0)
	    continue;
	...
	// Merge typed and mnvinfo history:
	// round 1: history of typed commands.
	// round 2: history from recently read mnvinfo.
	for (round = 1; round <= 2; ++round)
	{
	    if (round == 1)
		// start at newest entry, somewhere in the list
		i = *hisidx;
	    else if (mnvinfo_hisidx[type] > 0)
			...
			&& !(round == 2 && i >= mnvinfo_hisidx[type]))
		{
			...
			{
			    p = mnvinfo_history[type][i].hisstr;
			    plen = mnvinfo_history[type][i].hisstrlen;
			    timestamp = mnvinfo_history[type][i].time_set;
			}
		    }
    ...
    static void
write_mnvinfo_barlines(vir_T *virp, FILE *fp_out)
{
    int		i;
...
/*
 * Parse a mnvinfo line starting with '|'.
...
barline_parse(vir_T *virp, char_u *text, garray_T *values)
{
	...
	if (*p == '>')
	{
		...
		for (i = 0; i < values->ga_len; ++i)
		{
		    ...
		    {
			vp->bv_string = mnv_strnsave(vp->bv_string, vp->bv_len);
    ...
    static void
write_mnvinfo_version(FILE *fp_out)
{
    fprintf(fp_out, "# MNVinfo version\n|%d,%d\n\n",
					    BARTYPE_VERSION, MNVINFO_VERSION);
}

    static int
no_mnvinfo(void)
{
    // "mnv -i NONE" does not read or write a mnvinfo file
    return STRCMP(p_mnvinfofile, "NONE") == 0;
}

/*
 * Report an error for reading a mnvinfo file.
 * Count the number of errors.	When there are more than 10, return TRUE.
 */
    static int
mnvinfo_error(char *errnum, char *message, char_u *line)
{
    mnv_snprintf((char *)IObuff, IOSIZE, _("%smnvinfo: %s in line: "),
							     errnum, message);
    STRNCAT(IObuff, line, IOSIZE - STRLEN(IObuff) - 1);
    if (IObuff[STRLEN(IObuff) - 1] == '\n')
	IObuff[STRLEN(IObuff) - 1] = NUL;
    emsg((char *)IObuff);
    if (++mnvinfo_errcnt >= 10)
    {
	emsg(_(e_mnvinfo_too_many_errors_skipping_rest_of_file));
	return TRUE;
    }
    return FALSE;
}
...
/*
 * Compare the 'encoding' value in the mnvinfo file with the current value of
 * 'encoding'.  If different and the 'c' flag is in 'mnvinfo', setup for
 * conversion of text with iconv() in mnvinfo_readstring().
 */
    static int
mnvinfo_encoding(vir_T *virp)
{
    char_u	*p;
    int		i;

    if (get_mnvinfo_parameter('c') != 0)
    {
	p = mnv_strchr(virp->vir_line, '=');
	if (p != NULL)
	{
	    // remove trailing newline
	    ++p;
	    for (i = 0; mnv_isprintc(p[i]); ++i)
		;
	    p[i] = NUL;
...
/*
 * Restore global vars that start with a capital from the mnvinfo file
 */
    static int
read_mnvinfo_varlist(vir_T *virp, int writing)
{
    char_u	*tab;
    int		type = VAR_NUMBER;
    typval_T	tv;
    ...
    if (!writing && (find_mnvinfo_parameter('!') != NULL))
    {
	tab = mnv_strchr(virp->vir_line + 1, '\t');
	if (tab != NULL)
	{
	    *tab++ = '\0';	// isolate the variable name
		...
		if (type == VAR_STRING || type == VAR_DICT
			...
			|| type == VAR_TUPLE)
		    tv.vval.v_string = mnvinfo_readstring(virp,
...
/*
 * Write global vars that start with a capital to the mnvinfo file
 */
    static void
write_mnvinfo_varlist(FILE *fp)
{
	...
	if (!HASHITEM_EMPTY(hi))
	{
	    --todo;
	    this_var = HI2DI(hi);
	    if (var_flavour(this_var->di_key) == VAR_FLAVOUR_MNVINFO)
	    {
    ...
    static int
read_mnvinfo_sub_string(vir_T *virp, int force)
{
    if (force || get_old_sub() == NULL)
	set_old_sub(mnvinfo_readstring(virp, 1, TRUE));
    return mnvinfo_readline(virp);
}

    static void
write_mnvinfo_sub_string(FILE *fp)
{
    char_u *old_sub = get_old_sub();

    if (get_mnvinfo_parameter('/') == 0 || old_sub == NULL)
	return;

    fputs(_("\n# Last Substitute String:\n$"), fp);
    mnvinfo_writestring(fp, old_sub);
}

/*
 * Functions relating to reading/writing the search pattern from mnvinfo
 */

    static int
read_mnvinfo_search_pattern(vir_T *virp, int force)
{
    char_u	*lp;
    int		idx = -1;
    int		magic = FALSE;
	...
	{
	    val = mnvinfo_readstring(virp, (int)(lp - virp->vir_line + 1),
									TRUE);
	    if (val != NULL)
	    {
    ...
    int		sc)	// dir char
{
    ...
    fprintf(fp, "%c%c%c%c%ld%s%c",
	    ...
	    sc);
    mnvinfo_writestring(fp, spat->pat);
}

    static void
write_mnvinfo_search_pattern(FILE *fp)
{
    if (get_mnvinfo_parameter('/') == 0)
	return;

#ifdef FEAT_SEARCH_EXTRA
    fprintf(fp, "\n# hlsearch on (H) or off (h):\n~%c",
	    (no_hlsearch || find_mnvinfo_parameter('h') != NULL) ? 'h' : 'H');
#endif
...
/*
 * Functions relating to reading/writing registers from mnvinfo
 */
...
/*
 * Prepare for reading mnvinfo registers when writing mnvinfo later.
 */
    static void
prepare_mnvinfo_registers(void)
{
    y_read_regs = ALLOC_CLEAR_MULT(yankreg_T, NUM_REGISTERS);
}

    static void
finish_mnvinfo_registers(void)
{
    int		i;
    int		j;
	...
	if (y_read_regs[i].y_array != NULL)
	{
		...
		mnv_free(y_read_regs[i].y_array[j].string);
	    mnv_free(y_read_regs[i].y_array);
	}
    MNV_CLEAR(y_read_regs);
}

    static int
read_mnvinfo_register(vir_T *virp, int force)
{
    int		eof;
    int		do_it = TRUE;
    int		size;
    ...
    {
	if (mnvinfo_error("E577: ", _(e_illegal_register_name), virp->vir_line))
    ...
    while (!(eof = mnvinfo_readline(virp))
	...
	if (do_it)
	{
	    ...
	    str = mnvinfo_readstring(virp, 1, TRUE);
	    if (str != NULL)
	    {
	...
	for (i = 0; i < y_current_p->y_size; i++)
	    mnv_free(y_current_p->y_array[i].string);
	mnv_free(y_current_p->y_array);
...
/*
 * Accept a new style register line from the mnvinfo, store it when it's new.
 */
    static void
handle_mnvinfo_register(garray_T *values, int force)
{
    bval_T	*vp = (bval_T *)values->ga_data;
    int		flags;
    int		name;
    ...
    if (y_read_regs != NULL)
	// Reading mnvinfo for merging and writing.  Store the register
	...
	for (i = 0; i < y_ptr->y_size; i++)
	    mnv_free(y_ptr->y_array[i].string);
	...
	{
	    y_ptr->y_array[i].string = mnv_strnsave(vp[i + 6].bv_string, vp[i + 6].bv_len);
	    y_ptr->y_array[i].length = vp[i + 6].bv_len;
	}
    }
}
    ...
    static void
write_mnvinfo_registers(FILE *fp)
{
    int		i, j;
    char_u	*type;
    char_u	c;
    ...
    max_num_lines = get_mnvinfo_parameter('<');
    if (max_num_lines < 0)
	max_num_lines = get_mnvinfo_parameter('"');
    if (max_num_lines == 0)
	return;
    max_kbyte = get_mnvinfo_parameter('s');
    if (max_kbyte == 0)
	return;
	...
	if (i == TILDE_REGISTER)
	    continue;
#endif
	// When reading mnvinfo for merging and writing: Use the register from
	// mnvinfo if it's newer.
	if (y_read_regs != NULL
	...
	{
	    putc('\t', fp);
	    mnvinfo_writestring(fp, y_ptr->y_array[j].string);
	}
...
/*
 * Functions relating to reading/writing marks from mnvinfo
 */
...
write_buffer_marks(buf_T *buf, FILE *fp_out)
{
    ...
    fprintf(fp_out, "\n> ");
    mnvinfo_writestring(fp_out, IObuff);

    // Write the last used timestamp as the lnum of the non-existing mark '*'.
    // Older MNVs will ignore it and/or copy it.
 ...
 */
    static void
write_mnvinfo_marks(FILE *fp_out, garray_T *buflist)
{
    buf_T	*buf;
    int		is_mark_set;
    int		i;
    ...
    int		c2)
{
    ...
    static void
write_mnvinfo_filemarks(FILE *fp)
{
    int		i;
    char_u	*name;
    buf_T	*buf;
    ...
    if (curbuf->b_ffname != NULL && !skip_for_mnvinfo(curbuf))
    {
	...
	namedfm_p[NMARKS].time_set = mnv_time();
    }
...
/*
 * Handle marks in the mnvinfo file:
    ...
    int		flags)
{
    ...
    *name_buf = NUL;
	...
	list = list_alloc();
	if (list != NULL)
	    set_mnv_var_list(VV_OLDFILES, list);
    }
#endif

    num_marked_files = get_mnvinfo_parameter('\'');
	...
	if (line[0] != '>')
	{
	    if (line[0] != '\n' && line[0] != '\r' && line[0] != '#')
	    {
		if (mnvinfo_error("E576: ", _(e_nonr_missing_gt), line))
		    break;	// too many errors, return now
	    }
	    eof = mnv_fgets(line, LSIZE, virp->vir_fd);
	...
	if (str == NULL)
	    continue;
	p = str + STRLEN(str);
	while (p != str && (*p == NUL || mnv_isspace(*p)))
	    p--;
	if (*p)
	    p++;
	*p = NUL;
	...
	else // fp_out != NULL
	{
		...
		if (buflist_buf != NULL)
		{
		    ...
		    // newer.
		    if (!mnvinfo_readline(virp) && line[0] == TAB)
		    {
			did_read_line = TRUE;
			if (line[1] == '*')
			{
			...
			{
			    // No timestamp, must be written by an older MNV.
		...
		fputs("\n> ", fp_out);
		mnvinfo_writestring(fp_out, str);
		if (did_read_line)
	...
	pos.coladd = 0;
	while (!(eof = mnvinfo_readline(virp)) && line[0] == TAB)
	{
	    if (load_marks)
	    {
		if (line[1] != NUL)
...
/*
 * Read marks for the current buffer from the mnvinfo file, when we support
 * buffer marks and the buffer has a name.
 */
    void
check_marks_read(void)
{
    if (!curbuf->b_marks_read && get_mnvinfo_parameter('\'') > 0
						  && curbuf->b_ffname != NULL)
	read_mnvinfo(NULL, VIF_WANT_MARKS | VIF_ONLY_CURBUF);

    // Always set b_marks_read; needed when 'mnvinfo' is changed to include
    // the ' parameter after opening a buffer.
    curbuf->b_marks_read = TRUE;
}

    static int
read_mnvinfo_filemark(vir_T *virp, int force)
{
    char_u	*str;
    xfmark_T	*namedfm_p = get_namedfm();
    xfmark_T	*fm;
    ...
    if (*str <= 127
	    && ((*virp->vir_line == '\''
				       && (MNV_ISDIGIT(*str) || SAFE_isupper(*str)))
	     || (*virp->vir_line == '-' && *str == '\'')))
    {
	if (*str == '\'')
	{
	...
	if (fm != NULL && (fm->fmark.mark.lnum == 0 || force))
	{
	    ...
	    str = skipwhite(str);
	    mnv_free(fm->fname);
	    fm->fname = mnvinfo_readstring(virp, (int)(str - virp->vir_line),
								       FALSE);
	    fm->time_set = 0;
	}
    }
...
/*
 * Prepare for reading mnvinfo marks when writing mnvinfo later.
 */
    static void
prepare_mnvinfo_marks(void)
{
...
finish_mnvinfo_marks(void)
{
	...
	for (i = 0; i < NMARKS + EXTRA_MARKS; ++i)
	    mnv_free(vi_namedfm[i].fname);
	MNV_CLEAR(vi_namedfm);
    }
    if (vi_jumplist != NULL)
    {
	for (i = 0; i < vi_jumplist_len; ++i)
	    mnv_free(vi_jumplist[i].fname);
	MNV_CLEAR(vi_jumplist);
    }
}

/*
 * Accept a new style mark line from the mnvinfo, store it when it's new.
 */
    static void
handle_mnvinfo_mark(garray_T *values, int force)
{
    bval_T	*vp = (bval_T *)values->ga_data;
    int		name;
    linenr_T	lnum;
    ...
    name = vp[0].bv_nr;
    if (name != '\'' && !MNV_ISDIGIT(name) && !ASCII_ISUPPER(name))
	return;
    lnum = vp[1].bv_nr;
    col = vp[2].bv_nr;
    if (lnum <= 0 || col < 0)
	...
	if (MNV_ISDIGIT(name))
	{
		...
		// Do not use the name from the mnvinfo file, insert in time
		// order.
					  ...
					  || fm->time_set < timestamp || force)
	{
		...
		fm->fname = mnv_strsave(vp[4].bv_string);
    ...
    static int
read_mnvinfo_barline(vir_T *virp, int got_encoding, int force, int writing)
{
	...
	switch (bartype)
	{
		...
		// If it comes later it was copied by a MNV version that
		...
		read_next = barline_parse(virp, p, &values);
		handle_mnvinfo_history(&values, writing);
		break;

	    case BARTYPE_REGISTER:
		read_next = barline_parse(virp, p, &values);
		handle_mnvinfo_register(&values, force);
		break;

	    case BARTYPE_MARK:
		read_next = barline_parse(virp, p, &values);
		handle_mnvinfo_mark(&values, force);
		break;
	...
	for (i = 0; i < values.ga_len; ++i)
	{
	    ...
	    if (vp->bv_type == BVAL_STRING && vp->bv_allocated)
		mnv_free(vp->bv_string);
	    mnv_free(vp->bv_tofree);
	}
	ga_clear(&values);
    }
...
/*
 * read_mnvinfo_up_to_marks() -- Only called from do_mnvinfo().  Reads in the
 * first part of the mnvinfo file which contains everything but the marks that
 * are local to a file.  Returns TRUE when end-of-file is reached. -- webb
 */
    static int
read_mnvinfo_up_to_marks(
    vir_T	*virp,
    int		forceit,
    int		writing)
{
    ...
    prepare_mnvinfo_history(forceit ? 9999 : 0, writing);
	...
	switch (virp->vir_line[0])
	{
	    ...
	    case '+': // "+40 /path/dir file", for running mnv without args
	    case '^': // to be defined
		...
		break;
	    case '|':
		eof = read_mnvinfo_barline(virp, got_encoding,
							    forceit, writing);
		break;
	    case '*': // "*encoding=value"
		got_encoding = TRUE;
		eof = mnvinfo_encoding(virp);
		break;
	    case '!': // global variable
#ifdef FEAT_EVAL
		eof = read_mnvinfo_varlist(virp, writing);
#else
		eof = mnvinfo_readline(virp);
#endif
		break;
	    case '%': // entry for buffer list
		eof = read_mnvinfo_bufferlist(virp, writing);
		break;
	    case '"':
		// When registers are in bar lines skip the old style register
		// lines.
		if (virp->vir_version < MNVINFO_VERSION_WITH_REGISTERS)
		    eof = read_mnvinfo_register(virp, forceit);
		else
		    do
		    {
			eof = mnvinfo_readline(virp);
		...
		eof = read_mnvinfo_search_pattern(virp, forceit);
		break;
	    case '$':
		eof = read_mnvinfo_sub_string(virp, forceit);
		break;
	    case ':':
	    case '?':
	    case '=':
		...
		// lines.
		if (virp->vir_version < MNVINFO_VERSION_WITH_HISTORY)
		    eof = read_mnvinfo_history(virp, writing);
		else
		    eof = mnvinfo_readline(virp);
		break;
	    case '-':
		...
		if (virp->vir_version < MNVINFO_VERSION_WITH_MARKS)
		    eof = read_mnvinfo_filemark(virp, forceit);
		else
		    eof = mnvinfo_readline(virp);
		break;
	    default:
		if (mnvinfo_error("E575: ", _(e_illegal_starting_char),
			    virp->vir_line))
		    eof = TRUE;
		else
		    eof = mnvinfo_readline(virp);
...
/*
 * do_mnvinfo() -- Should only be called from read_mnvinfo() & write_mnvinfo().
 */
    static void
do_mnvinfo(FILE *fp_in, FILE *fp_out, int flags)
{
    int		eof = FALSE;
    vir_T	vir;
    int		merge = FALSE;
	...
	if (flags & VIF_WANT_INFO)
	{
		...
		// this MNV is using.  They are merged when writing.
		prepare_mnvinfo_registers();
		prepare_mnvinfo_marks();
	    }

	    eof = read_mnvinfo_up_to_marks(&vir,
					 flags & VIF_FORCEIT, fp_out != NULL);
	    merge = TRUE;
	}
	else if (flags != 0)
	    // Skip info, find start of marks
	    while (!(eof = mnvinfo_readline(&vir))
		    && vir.vir_line[0] != '>')
		;
    ...
    {
	// Write the info:
	fprintf(fp_out, _("# This mnvinfo file was generated by MNV %s.\n"),
							  MNV_VERSION_MEDIUM);
	fputs(_("# You may edit it if you're careful!\n\n"), fp_out);
	write_mnvinfo_version(fp_out);
	fputs(_("# Value of 'encoding' when this file was written\n"), fp_out);
	fprintf(fp_out, "*encoding=%s\n\n", p_enc);
	write_mnvinfo_search_pattern(fp_out);
	write_mnvinfo_sub_string(fp_out);
	write_mnvinfo_history(fp_out, merge);
	write_mnvinfo_registers(fp_out);
	finish_mnvinfo_registers();
#ifdef FEAT_EVAL
	write_mnvinfo_varlist(fp_out);
#endif
	write_mnvinfo_filemarks(fp_out);
	finish_mnvinfo_marks();
	write_mnvinfo_bufferlist(fp_out);
	write_mnvinfo_barlines(&vir, fp_out);

	if (do_copy_marks)
	    ga_init2(&buflist, sizeof(buf_T *), 50);
	write_mnvinfo_marks(fp_out, do_copy_marks ? &buflist : NULL);
    }

    if (do_copy_marks)
    {
	copy_mnvinfo_marks(&vir, fp_out, &buflist, eof, flags);
	if (fp_out != NULL)
	    ga_clear(&buflist);
    }

    mnv_free(vir.vir_line);
...
/*
 * read_mnvinfo() -- Read the mnvinfo file.  Registers etc. which are already
    ...
    int		flags)	    // VIF_WANT_INFO et al.
{
    FILE	*fp;
    char_u	*fname;
    stat_T	st;		// mch_stat() of existing mnvinfo file

    if (no_mnvinfo())
	return FAIL;

    fname = mnvinfo_filename(file);	// get file name in allocated buffer
    if (fname == NULL)
	return FAIL;
    ...
    {
	verbose_enter();
	smsg(_("Reading mnvinfo file \"%s\"%s%s%s%s"),
		fname,
...
/*
 * Write the mnvinfo file.  The old one is read in first so that effectively a
 * merge of current info and old info is done.  This allows multiple mnvs to
 * run simultaneously, without losing any marks etc.
 * If "forceit" is TRUE, then the old file is not read in, and only internal
 * info is written to the file.
 */
    void
write_mnvinfo(char_u *file, int forceit)
{
    char_u	*fname;
    FILE	*fp_in = NULL;	// input mnvinfo file, if any
    FILE	*fp_out = NULL;	// output mnvinfo file
    char_u	*tempname = NULL;	// name of temp mnvinfo file
    stat_T	st_new;		// mch_stat() of potential new file
    stat_T	st_old;		// mch_stat() of existing mnvinfo file
    ...
    fname = mnvinfo_filename(file);	// may set to default if NULL
    if (fname == NULL)
	return;
	...
	// Create the new .mnvinfo non-accessible for others, because it may
    ...
    {
	// There is an existing mnvinfo file.  Create a temporary file to
	// write the new mnvinfo into, in the same directory as the
	// existing mnvinfo file, which will be renamed once all writing is
	// successful.
	if (mch_fstat(fileno(fp_in), &st_old) < 0
		|| S_ISDIR(st_old.st_mode)
#ifdef UNIX
		// For Unix we check the owner of the file.  It's not very nice
		// to overwrite a user's mnvinfo file after a "su root", with a
		// mnvinfo file that the user can't read.
		|| (getuid() != ROOT_UID
		...
		)
	{
	    ...
	    semsg(_(e_mnvinfo_file_is_not_writable_str), fname);
	    msg_didany = tt;
	    fclose(fp_in);
	    goto end;
	}
#ifdef MSWIN
	// Get the file attributes of the existing mnvinfo file.
	hidden = mch_ishidden(fname);
#endif

	// Make tempname, find one that does not exist yet.
	// Beware of a race condition: If someone logs out and all MNV
	...
	// case somebody puts his mnvinfo file in an 8.3 filesystem.
	for (;;)
	{
	    int		next_char = 'z';
	    char_u	*wp;
		...
		else
		{
		    ...
		    // another MNV tries to do it at the same time.
#ifdef VMS
...
# endif
		...
		if (next_char == 'a' - 1)
		{
		    ...
		    // the mnvinfo file then.
		    semsg(_(e_too_many_mnvinfo_temp_files_like_str), tempname);
		    break;
		}
		*wp = next_char;
		--next_char;
	...
	if (tempname != NULL && fp_out != NULL)
	{
		...
		if (st_old.st_uid != tmp_st.st_uid)
		    ...
		    mnv_ignored = fchown(fileno(fp_out), st_old.st_uid, -1);
    ...
    // Check if the new mnvinfo file can be written to.
    if (fp_out == NULL)
    {
	semsg(_(e_cant_write_mnvinfo_file_str),
    ...
    {
	verbose_enter();
	smsg(_("Writing mnvinfo file \"%s\""), fname);
	verbose_leave();
    }

    mnvinfo_errcnt = 0;
    do_mnvinfo(fp_in, fp_out, forceit ? 0 : (VIF_WANT_INFO | VIF_WANT_MARKS));
	...
	// In case of an error keep the original mnvinfo file.  Otherwise
	// rename the newly written file.  Give an error if that fails.
	if (mnvinfo_errcnt == 0)
	{
	    if (mnv_rename(tempname, fname) == -1)
	    {
		++mnvinfo_errcnt;
		semsg(_(e_cant_rename_mnvinfo_file_to_str), fname);
	    }
#ifdef MSWIN
	    // If the mnvinfo file was hidden then also hide the new file.
	    else if (hidden)
		mch_hide(fname);
#endif
	}
    ...
    exarg_T	*eap)
{
    ...
    save_mnvinfo = p_mnvinfo;
    if (*p_mnvinfo == NUL)
	p_mnvinfo = (char_u *)"'100";
    if (eap->cmdidx == CMD_rmnvinfo)
    {
	if (read_mnvinfo(eap->arg, VIF_WANT_INFO | VIF_WANT_MARKS
				  | (eap->forceit ? VIF_FORCEIT : 0)) == FAIL)
	    emsg(_(e_cannot_open_mnvinfo_file_for_reading));
    }
    else
	write_mnvinfo(eap->arg, eap->forceit);
    p_mnvinfo = save_mnvinfo;
}
