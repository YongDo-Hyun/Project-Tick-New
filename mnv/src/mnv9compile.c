...
/*
 * mnv9compile.c: compiling a :def function
 */
	...
	type_T	*type)
{
    ...
    lvar->lv_name = mnv_strnsave(name, len == 0 ? STRLEN(name) : len);
...
may_peek_next_line(cctx_T *cctx, char_u *arg, char_u **nextp)
{
    ...
    *nextp = NULL;
    if (*p == NUL || (MNV_ISWHITE(*arg) && mnv9_comment_start(p)))
    {
...
next_line_from_context(cctx_T *cctx, int skip_comment)
{
	...
	SOURCING_LNUM = cctx->ctx_lnum + 1;
		     ...
		     || (skip_comment && mnv9_comment_start(skipwhite(line))));
    return line;
}
...
may_get_next_line(char_u *whitep, char_u **arg, cctx_T *cctx)
{
    *arg = skipwhite(whitep);
    if (mnv9_bad_comment(*arg))
	return FAIL;
    if (**arg == NUL || (MNV_ISWHITE(*whitep) && mnv9_comment_start(*arg)))
    {
...
compile_nested_function(exarg_T *eap, cctx_T *cctx, garray_T *lines_to_free)
{
    ...
    lambda_name.string = mnv_strnsave(lambda_name.string, lambda_name.length);
    if (lambda_name.string == NULL)
	return NULL;

    // This may free the current line, make a copy of the name.
    off = is_global ? 2 : 0;
    func_name = mnv_strnsave(name_start + off, name_end - name_start - off);
    if (func_name == NULL)
    {
	r = FAIL;
	goto theend;
...
theend:
    mnv_free(lambda_name.string);
    mnv_free(func_name);
    return r == FAIL ? NULL : (char_u *)"";
}

/*
 * Compile one MNV expression {expr} in string "p".
 ...
 * literal strings and MNV expressions of the form `{expr}`).  This is used
 * when compiling a heredoc assignment to a variable or an interpolated string
 * in a MNV9 def function.  MNV9 instructions are generated to push strings,
 * evaluate expressions, concatenate them and create a list of lines.  When
 * "evalstr" is TRUE, MNV expressions in "str" are evaluated.
 */
    int
compile_all_expr_in_str(char_u *str, int evalstr, cctx_T *cctx)
{
	...
	val = *str != NUL ? mnv_strsave(str) : NULL;
	...
	if (p != lit_start)
	{
	    val = mnv_strnsave(lit_start, (size_t)(p - lit_start));
...
assignment_len(char_u *p, int *heredoc)
{
    ...
    if (mnv_strchr((char_u *)"+-*/%", *p) != NULL && p[1] == '=')
	return 2;
    if (STRNCMP(p, "..=", 3) == 0)
	return 3;
    return 0;
...
skip_index(char_u *start)
{
    ...
    void
mnv9_declare_error(char_u *name)
{
    char *scope = "";
	...
	cctx_T		*cctx)
{
	...
	*mnvvaridx = find_mnv_var(name + 2, &di_flags);
	if (*mnvvaridx < 0)
	{
	...
	if (var_check_ro(di_flags, name, FALSE))
	    return FAIL;
	*dest = dest_mnvvar;
	vtv = get_mnv_var_tv(*mnvvaridx);
	*type = typval2type_mnvvar(vtv, cctx->ctx_type_list);
    }
    return OK;
}
...
lhs_class_member_modifiable(lhs_T *lhs, char_u	*var_start, cctx_T *cctx)
{
    ...
    if ((m->ocm_access != MNV_ACCESS_ALL) &&
	    ((is_object && !inside_class(cctx, cl))
	     || (!is_object && cctx->ctx_ufunc->uf_class != cl)))
    {
	char *msg = (m->ocm_access == MNV_ACCESS_PRIVATE)
    ...
    char_u	**var_endp)
{
    ...
    lhs->lhs_name = mnv_strnsave(var_start, lhs->lhs_varlen);
...
{
    char_u	*var_end = *var_endp;
    char_u	*dot = mnv_strchr(var_start, '.');
    char_u	*p;
    ...
    mnv_free(lhs->lhs_name);
    lhs->lhs_varlen = var_end - p;
    lhs->lhs_name = mnv_strnsave(p, lhs->lhs_varlen);
    if (lhs->lhs_name == NULL)
	return FAIL;
    ...
    int		is_decl)
{
	...
	if (script_namespace && current_script_is_mnv9())
	{
	    semsg(_(e_cannot_use_s_colon_in_mnv9_script_str), var_start);
	    return FAIL;
	}
	...
	else if (cctx->ctx_ufunc->uf_script_ctx_version == SCRIPT_VERSION_MNV9
		&& script_namespace
	...
	lhs->lhs_dest = current_script_is_mnv9() ? dest_script_v9 :
								dest_script;
    ...
    int		is_decl)
{
    ...
    if (get_var_dest(lhs->lhs_name, &lhs->lhs_dest, cmdidx,
				&lhs->lhs_opt_flags, &lhs->lhs_mnvvaridx,
    ...
    if (declare_error)
    {
	mnv9_declare_error(lhs->lhs_name);
	return FAIL;
    }
	...
	cctx_T	*cctx)
{
	...
	char_u *key = mnv_strnsave(p + 1, key_end - p - 1);
    ...
    char_u	**retstr)
{
	...
	*retstr = arg;
    ...
    if (cac->cac_oplen > 0 && (!MNV_ISWHITE(*sp)
...
add_def_function(ufunc_T *ufunc)
{
    ...
    dfunc->df_name = mnv_strnsave(ufunc->uf_name, ufunc->uf_namelen);
    ...
    cctx_T	*cctx)
{
	...
	else if (line != NULL && mnv9_bad_comment(skipwhite(line)))
	    return FAIL;
	else
	{
	...
	switch (ea.cmdidx)
	{
	    ...
	    case CMD_mnv9script:
		    if (cctx->ctx_skip != SKIP_YES)
		    {
			emsg(_(e_mnv9script_can_only_be_used_in_script));
			return FAIL;
		    }
		    line = (char_u *)"";
		    break;
...
erret:
	...
	mnv_free(cctx.ctx_redir_lhs.lhs_name);
	mnv_free(cctx.ctx_redir_lhs.lhs_whole);
    }
