...
namespace Commandline
{
	...
	class Parser
	{
	  public:
		 ...
		 * @param def the default value
		 */
		void addArgument(QString name, bool required = true,
		 ...
		 * Note: on positional arguments, metavar replaces the name as
		 * displayed. on options , metavar replaces the value placeholder
		 */
		void addDocumentation(QString name, QString doc,
							  QString metavar = QString());

		/**
		 * @brief generate a help message
		 * @param progName the program name to use in the help message
		 * @param helpIndent how much the parameter documentation should be
		 * indented
		 * @param flagsInUsage whether we should use flags instead of options in
		 * the usage
		 * @return a help message
		 */
		QString compileHelp(QString progName, int helpIndent = 22,
							bool flagsInUsage = true);
	  ...
	  private:
		...
		enum OptionType { otSwitch, otOption };
