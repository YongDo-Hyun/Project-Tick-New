...
QVector<PluginMetadata> PluginLoader::discoverModules() const
{
    ...
    for (const QString& dir : searchPaths()) {
        for (auto& meta : scanDirectory(dir)) {
            ...
            if (seen.contains(id)) {
                qDebug() << "[PluginLoader] Skipping duplicate module" << id << "from" << meta.filePath;
...
PluginMetadata PluginLoader::loadModule(const QString& path) const
{
...
#ifdef Q_OS_WIN
    auto* info = reinterpret_cast<MMCOModuleInfo*>(
        GetProcAddress(static_cast<HMODULE>(meta.libraryHandle), "mmco_module_info"));
#else
    ...
    if (info->magic != MMCO_MAGIC) {
        qWarning() << "[PluginLoader]" << path << "bad magic:" << Qt::hex << info->magic
                    << "(expected" << Qt::hex << MMCO_MAGIC << ")";
        unloadModule(meta);
        return meta;
    }
    ...
    if (info->abi_version != MMCO_ABI_VERSION) {
        qWarning() << "[PluginLoader]" << path << "ABI version mismatch:" << info->abi_version
    ...
    meta.unloadFunc = reinterpret_cast<PluginMetadata::UnloadFunc>(
        GetProcAddress(static_cast<HMODULE>(meta.libraryHandle), "mmco_unload"));
#else
    ...
    meta.loaded = true;
    qDebug() << "[PluginLoader] Loaded module:" << meta.name
