...
void PluginManager::initializeAll()
{
    ...
    for (int i = 0; i < m_modules.size(); ++i) {
        ...
        if (rc != 0) {
            ...
            emit moduleError(meta.name, QString("mmco_init returned %1").arg(rc));
            PluginLoader::unloadModule(meta);
            continue;
        }

        meta.initialized = true;
        qDebug() << "[PluginManager] Module" << meta.name << "initialized successfully";
...
const char* PluginManager::api_setting_get(void* mh, const char* key)
{
    ...
    auto& meta = r->manager->m_modules[r->moduleIndex];
    QString fullKey = QString("plugin.%1.%2").arg(meta.moduleId(), QString::fromUtf8(key));
...
static BaseInstance* resolveInstance(PluginManager::ModuleRuntime* r, const char* id)
{
    auto* app = r->manager->m_app;
...
static MinecraftInstance* resolveMC(PluginManager::ModuleRuntime* r, const char* id)
{
    return dynamic_cast<MinecraftInstance*>(resolveInstance(r, id));
}

int PluginManager::api_instance_set_name(void* mh, const char* id, const char* name)
{
    auto* r = rt(mh);
...
int PluginManager::api_instance_set_icon_key(void* mh, const char* id, const char* key)
{
    auto* r = rt(mh);
    auto* inst = resolveInstance(r, id);
    if (!inst || !key)
...
int PluginManager::api_instance_set_notes(void* mh, const char* id, const char* notes)
{
    auto* r = rt(mh);
    auto* inst = resolveInstance(r, id);
    if (!inst || !notes)
...
int64_t PluginManager::api_instance_get_total_play_time(void* mh, const char* id)
{
    auto* r = rt(mh);
    auto* inst = resolveInstance(r, id);
...
const char* PluginManager::api_instance_get_group(void* mh, const char* id)
{
    ...
    if (!app || !app->instances() || !id)
        return nullptr;
    r->tempString = app->instances()->getInstanceGroup(QString::fromUtf8(id)).toStdString();
    return r->tempString.c_str();
}

int PluginManager::api_instance_set_group(void* mh, const char* id, const char* group)
{
    auto* r = rt(mh);
    auto* app = r->manager->m_app;
    if (!app || !app->instances() || !id)
        return -1;
    app->instances()->setInstanceGroup(QString::fromUtf8(id),
                                       group ? QString::fromUtf8(group) : QString());
    return 0;
}
...
const char* PluginManager::api_instance_component_get_uid(void* mh, const char* id, int idx)
{
    auto* r = rt(mh);
    auto* mc = resolveMC(r, id);
    if (!mc || !mc->getPackProfile())
...
const char* PluginManager::api_instance_component_get_name(void* mh, const char* id, int idx)
{
    auto* r = rt(mh);
    auto* mc = resolveMC(r, id);
    if (!mc || !mc->getPackProfile())
...
const char* PluginManager::api_instance_component_get_version(void* mh, const char* id, int idx)
{
    auto* r = rt(mh);
    auto* mc = resolveMC(r, id);
    if (!mc || !mc->getPackProfile())
...
const char* PluginManager::api_instance_get_mc_version(void* mh, const char* id)
{
    ...
    if (!mc || !mc->getPackProfile())
        return nullptr;
    r->tempString = mc->getPackProfile()->getComponentVersion("net.minecraft").toStdString();
    return r->tempString.c_str();
}

const char* PluginManager::api_instance_get_jar_mods_dir(void* mh, const char* id)
{
    auto* r = rt(mh);
    auto* mc = resolveMC(r, id);
    if (!mc)
...
const char* PluginManager::api_instance_get_resource_packs_dir(void* mh, const char* id)
{
    auto* r = rt(mh);
    auto* mc = resolveMC(r, id);
    if (!mc)
...
const char* PluginManager::api_instance_get_texture_packs_dir(void* mh, const char* id)
{
    auto* r = rt(mh);
    auto* mc = resolveMC(r, id);
    if (!mc)
...
const char* PluginManager::api_instance_get_shader_packs_dir(void* mh, const char* id)
{
    auto* r = rt(mh);
    auto* mc = resolveMC(r, id);
    if (!mc)
...
int64_t PluginManager::api_fs_read(void* mh, const char* rel,
                                   void* buf, size_t sz)
{
    ...
    auto* r = rt(mh);
    QString path = QDir(QString::fromStdString(r->dataDir)).filePath(QString::fromUtf8(rel));
                                ...
                                const void* data, size_t sz)
{
    ...
    qint64 written = f.write(static_cast<const char*>(data), static_cast<qint64>(sz));
                                 ...
                                 MMCOHttpCallback cb, void* ud)
{
    ...
    QByteArray postData(static_cast<const char*>(body), static_cast<int>(body_sz));
    QNetworkReply* reply = nam->post(request, postData);

    QObject::connect(reply, &QNetworkReply::finished, [reply, cb, ud]() {
        int status = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
        QByteArray respBody = reply->readAll();
        cb(ud, status, respBody.constData(), static_cast<size_t>(respBody.size()));
        reply->deleteLater();
    });
...
void PluginManager::api_ui_show_message(void* mh, int type,
                                        const char* title, const char* msg)
{
    auto* r = rt(mh);
    auto& meta = r->manager->m_modules[r->moduleIndex];

    QString qtitle = QString("[%1] %2").arg(meta.name, QString::fromUtf8(title));
                                        ...
                                        void* ud)
{
    ...
    QObject::connect(action, &QAction::triggered, [cb, ud]() {
        cb(ud);
    });
...
int PluginManager::api_zip_compress_dir(void* mh, const char* zip, const char* dir)
{
    (void)mh;
    if (!zip || !dir)
        return -1;
...
int PluginManager::api_zip_extract(void* mh, const char* zip, const char* target)
{
    (void)mh;
    if (!zip || !target)
        return -1;
    auto result = MMCZip::extractDir(QString::fromUtf8(zip),
...
static std::shared_ptr<ModFolderModel> resolveModList(PluginManager::ModuleRuntime* r,
                                                       const char* inst_id,
                                                       const char* type)
{
                                        ...
                                        const char* type, int idx, int e)
{
    ...
    return model->setModStatus(indices, e ? ModFolderModel::Enable
                                          : ModFolderModel::Disable) ? 0 : -1;
}

int PluginManager::api_mod_remove(void* mh, const char* inst,
                                   const char* type, int idx)
{
    auto* r = rt(mh);
    auto model = resolveModList(r, inst, type);
...
static std::shared_ptr<WorldList> resolveWorldList(PluginManager::ModuleRuntime* r,
                                                    const char* inst_id)
{
    auto* mc = resolveMC(r, inst_id);
    if (!mc)
...
const char* PluginManager::api_world_get_name(void* mh, const char* inst, int idx)
{
    auto* r = rt(mh);
    auto wl = resolveWorldList(r, inst);
...
const char* PluginManager::api_world_get_folder(void* mh, const char* inst, int idx)
{
    auto* r = rt(mh);
    auto wl = resolveWorldList(r, inst);
    if (!wl || idx < 0 || idx >= static_cast<int>(wl->size()))
...
int64_t PluginManager::api_world_get_last_played(void* mh, const char* inst, int idx)
{
    auto* r = rt(mh);
    auto wl = resolveWorldList(r, inst);
    if (!wl || idx < 0 || idx >= static_cast<int>(wl->size()))
...
int PluginManager::api_world_rename(void* mh, const char* inst, int idx, const char* name)
{
    auto* r = rt(mh);
    auto wl = resolveWorldList(r, inst);
...
int PluginManager::api_world_install(void* mh, const char* inst, const char* path)
{
    auto* r = rt(mh);
    auto wl = resolveWorldList(r, inst);
    if (!wl || !path)
...
const char* PluginManager::api_instance_get_java_version(void* mh, const char* id)
{
    auto* r = rt(mh);
    auto* mc = resolveMC(r, id);
    if (!mc)
...
int PluginManager::api_ui_confirm_dialog(void* mh, const char* title, const char* msg)
{
    (void)mh;
                                         ...
                                         const char* name, const char* iconName)
{
    ...
    if (!id || !name)
        return nullptr;
    auto* page = new PluginPage(
        QString::fromUtf8(id),
        QString::fromUtf8(name),
        iconName ? QString::fromUtf8(iconName) : QStringLiteral("plugin"));
...
int PluginManager::api_ui_layout_add_widget(void* mh, void* layout, void* widget)
{
    (void)mh;
    if (!layout || !widget)
        return -1;
...
int PluginManager::api_ui_layout_add_spacer(void* mh, void* layout, int horizontal)
{
    (void)mh;
    if (!layout)
        return -1;
    ...
    if (horizontal)
        ...
        l->addItem(new QSpacerItem(0, 0, QSizePolicy::Minimum, QSizePolicy::Expanding));
    return 0;
}
...
void* PluginManager::api_ui_button_create(void* mh, void* parent, const char* text,
                                           const char* iconName,
...
int PluginManager::api_ui_button_set_text(void* mh, void* btn, const char* text)
{
    (void)mh;
    if (!btn) return -1;
    static_cast<QPushButton*>(btn)->setText(text ? QString::fromUtf8(text) : QString());
    return 0;
}

void* PluginManager::api_ui_label_create(void* mh, void* parent, const char* text)
{
    (void)mh;
...
int PluginManager::api_ui_label_set_text(void* mh, void* label, const char* text)
{
    (void)mh;
    if (!label) return -1;
    static_cast<QLabel*>(label)->setText(text ? QString::fromUtf8(text) : QString());
    return 0;
}
    ...
    if (cb) {
        QObject::connect(tree, &QTreeWidget::itemSelectionChanged,
                         [tree, cb, ud]() {
                             ...
                             int row = items.isEmpty() ? -1 : tree->indexOfTopLevelItem(items.first());
                             cb(ud, row);
                         });
    }
    return tree;
...
int PluginManager::api_ui_tree_set_row_data(void* mh, void* tree, int row, int64_t data)
{
    (void)mh;
    if (!tree) return -1;
    auto* tw = static_cast<QTreeWidget*>(tree);
